...
char WizardDestInstallPath [GST_MAX_PATH];
char WizardDestExtractPath [GST_MAX_PATH];
char SelfFile [GST_MAX_PATH];
	...
	case WM_INITDIALOG:
		...
		case INTRO_PAGE:
			{
				...
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_INFO), L"You must accept these license terms before you can use, extract, or install GostCrypt.");
		...
		case EXTRACTION_OPTIONS_PAGE:
			...
			{ 
				...
				strncat (WizardDestExtractPath, "GostCrypt\\", sizeof (WizardDestExtractPath) - strlen (WizardDestExtractPath) - 1);
			}

			SendMessage (GetDlgItem (hwndDlg, IDC_DESTINATION), EM_LIMITTEXT, GST_MAX_PATH - 1, 0);
		...
		case INSTALL_OPTIONS_PAGE:
			{
				...
				if (bUpgrade)
				{
					...
					bForAllUsers = (_access ((string (path) + "\\" GST_APP_NAME).c_str(), 0) == 0);
				}
		...
		case DONATIONS_PAGE:
			...
			DonText = L"If you want to make a donation, please contact us.";
...
BOOL CALLBACK MainDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			SetWindowText (hwndDlg, "GostCrypt Setup " VERSION_STRING);
	...
	case WM_COMMAND:
		...
		if (lw == IDC_NEXT)
		{
			if (nCurPageNo == INTRO_PAGE)
			{
				...
				{
					WarningDirect (L"Warning: Please note that this may be the last version of GostCrypt that supports Windows 2000. If you want to be able to upgrade to future versions of GostCrypt (which is highly recommended), you will need to upgrade to Windows XP or a later version of Windows.\n\nNote: Microsoft stopped issuing security updates for Windows 2000 to the general public on 7/13/2010 (the last non-security update for Windows 2000 was issued to the general public in 2005).");
					...
					{
						ErrorDirect (L"GostCrypt requires Update Rollup 1 for Windows 2000 SP4 to be installed.\n\nFor more information, see http://support.microsoft.com/kb/891861");
						AbortProcessSilent ();
					}
	...
	case GST_APPMSG_INSTALL_SUCCESS:
	...
	case GST_APPMSG_INSTALL_FAILURE:
	...
	case GST_APPMSG_EXTRACTION_SUCCESS:
	...
	case GST_APPMSG_EXTRACTION_FAILURE:
