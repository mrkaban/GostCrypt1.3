...
char InstallationPath[GST_MAX_PATH];
char SetupFilesDir[GST_MAX_PATH];
	    ...
	    char *lpszPathLink)
{
	...
	if (SUCCEEDED (hres))
	{
		...
		// Application ID
		if (strstr (lpszPathObj, GST_APP_NAME ".exe"))
		{
			...
			if (SUCCEEDED (psl->QueryInterface (IID_PPV_ARGS (&propStore))))
			{
				...
				if (SUCCEEDED (InitPropVariantFromString (GST_APPLICATION_ID, &propVariant)))
				{
...
void DetermineUpgradeDowngradeStatus (BOOL bCloseDriverHandle, LONG *driverVersionPtr)
{
	...
	if (hDriver != INVALID_HANDLE_VALUE)
	{
		...
		PortableMode = DeviceIoControl (hDriver, GST_IOCTL_GET_PORTABLE_MODE_STATUS, NULL, 0, NULL, 0, &dwResult, NULL);
...
BOOL DoFilesInstall (HWND hwndDlg, char *szDestDir)
{
	...
	char szTmp[GST_MAX_PATH];
	BOOL bOK = TRUE;
	int i, x, fileNo;
	char curFileName [GST_MAX_PATH] = {0};
	...
	for (i = 0; i < sizeof (szFiles) / sizeof (szFiles[0]); i++)
	{
		...
		if (strstr (szFiles[i], "GostCrypt Setup") != 0)
		{
			if (bUninstall)
		...
		if (bUninstall == FALSE)
		{
			...
			if (strstr (szFiles[i], "GostCrypt Setup") != 0)
			{
				// Copy ourselves (the distribution package) to the destination location as 'GostCrypt Setup.exe'

				char mp[MAX_PATH];

				GetModuleFileName (NULL, mp, sizeof (mp));
				bResult = GSTCopyFile (mp, szTmp);
			}
			else
			{
				BOOL driver64 = FALSE;
				...
				if (Is64BitOs ()
					&& strcmp (szFiles[i], "Dgostcrypt.sys") == 0)
				{
					driver64 = TRUE;
					...
					for (fileNo = 0; fileNo < NBR_COMPRESSED_FILES; fileNo++)
					{
							...
							min (strlen (curFileName), (size_t) Decompressed_Files[fileNo].fileNameLength)) == 0)
						{
							...
							if (driver64)
							{
								...
								{
									bResult = WriteLocalMachineRegistryString ("SYSTEM\\CurrentControlSet\\Services\\gostcrypt", "ImagePath", "System32\\drivers\\gostcrypt.sys", TRUE);
									if (!bResult)
									{
										handleWin32Error (hwndDlg);
										goto err;
				...
				else
				{
					...
					bResult = GSTCopyFile (curFileName, szTmp);

					if (driver64)
						EnableWow64FsRedirection (TRUE);
				}

				if (bResult && strcmp (szFiles[i], "AGostCrypt.exe") == 0)
				{
...
err:
	...
	if (bUninstall == FALSE)
	{
		...
		if (h != INVALID_HANDLE_VALUE)
		{
			...
			GSTCopyFile (f.cFileName, d);
			FindClose (h);
		}

		SetCurrentDirectory (SetupFilesDir);
		SetCurrentDirectory ("Setup files");
		h = FindFirstFile ("GostCrypt User Guide.*.pdf", &f);
		if (h != INVALID_HANDLE_VALUE)
		{
			char d[MAX_PATH*2];
			sprintf (d, "%s%s", szDestDir, f.cFileName);
...
{
	char szDir[GST_MAX_PATH], *key;
	char szTmp[GST_MAX_PATH*4];
	HKEY hkey = 0;
	...
	{
		if (RegCreateKeyEx (HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GostCrypt",
	...
	if (bInstallType)
	{
		...
		key = "Software\\Classes\\GostCryptVolume";
		...
		key = "Software\\Classes\\GostCryptVolume\\DefaultIcon";
		RegMessage (hwndDlg, key);
		if (RegCreateKeyEx (HKEY_LOCAL_MACHINE,
				    key,
				    0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkey, &dw) != ERROR_SUCCESS)
		...
		sprintf (szTmp, "%sGostCrypt.exe,1", szDir);
		if (RegSetValueEx (hkey, "", 0, REG_SZ, (BYTE *) szTmp, strlen (szTmp) + 1) != ERROR_SUCCESS)
			goto error;
		...
		key = "Software\\Classes\\GostCryptVolume\\Shell\\open\\command";
		RegMessage (hwndDlg, key);
		if (RegCreateKeyEx (HKEY_LOCAL_MACHINE,
				    key,
				    0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkey, &dw) != ERROR_SUCCESS)
		...
		sprintf (szTmp, "\"%sGostCrypt.exe\" /v \"%%1\"", szDir );
		if (RegSetValueEx (hkey, "", 0, REG_SZ, (BYTE *) szTmp, strlen (szTmp) + 1) != ERROR_SUCCESS)
			goto error;
		...
		key = "Software\\Classes\\.gst";
		BOOL typeClassChanged = TRUE;
		char typeClass[256];
		DWORD typeClassSize = sizeof (typeClass);

		if (ReadLocalMachineRegistryString (key, "", typeClass, &typeClassSize) && typeClassSize > 0 && strcmp (typeClass, "GostCryptVolume") == 0)
	...
	key = "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GostCrypt";
	RegMessage (hwndDlg, key);
	if (RegCreateKeyEx (HKEY_LOCAL_MACHINE,
		key,
		0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkey, &dw) != ERROR_SUCCESS)
...
BOOL DoApplicationDataUninstall (HWND hwndDlg)
{
	...
	sprintf (path2, "%s%s", path, GST_APPD_FILENAME_FAVORITE_VOLUMES);
	RemoveMessage (hwndDlg, path2);
	StatDeleteFile (path2);

	// Delete keyfile defaults
	sprintf (path2, "%s%s", path, GST_APPD_FILENAME_DEFAULT_KEYFILES);
	RemoveMessage (hwndDlg, path2);
	StatDeleteFile (path2);

	// Delete history file
	sprintf (path2, "%s%s", path, GST_APPD_FILENAME_HISTORY);
	RemoveMessage (hwndDlg, path2);
	StatDeleteFile (path2);
	
	// Delete configuration file
	sprintf (path2, "%s%s", path, GST_APPD_FILENAME_CONFIGURATION);
	RemoveMessage (hwndDlg, path2);
	StatDeleteFile (path2);

	// Delete system encryption configuration file
	sprintf (path2, "%s%s", path, GST_APPD_FILENAME_SYSTEM_ENCRYPTION);
	RemoveMessage (hwndDlg, path2);
	StatDeleteFile (path2);
...
BOOL DoRegUninstall (HWND hwndDlg, BOOL bRemoveDeprecated)
{
	...
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GostCrypt");
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\GostCryptVolume\\Shell\\open\\command");
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\GostCryptVolume\\Shell\\open");
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\GostCryptVolume\\Shell");
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\GostCryptVolume\\DefaultIcon");
	RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\GostCryptVolume");
	RegDeleteKey (HKEY_CURRENT_USER, "Software\\GostCrypt");
	...
	if (!bRemoveDeprecated)
	{
		...
		RegDeleteKey (HKEY_LOCAL_MACHINE, "Software\\Classes\\.gst");
		SHChangeNotify (SHCNE_ASSOCCHANGED, SHCNF_IDLIST, NULL, NULL);
	}
...
retry:
	...
	if (strcmp ("gostcrypt", lpszService) == 0)
	{
		try
		{
...
BOOL DoDriverUnload (HWND hwndDlg)
{
	...
	if (hDriver != INVALID_HANDLE_VALUE)
	{
			...
			if (bootEnc.GetDriverServiceStartType() == SERVICE_BOOT_START)
			{
				try
				{
					...
					if (IsHiddenOSRunning())
					{
						...
						{
							if (AskWarnNoYes ("UPDATE_GST_IN_DECOY_OS_FIRST") == IDNO)
								AbortProcessSilent ();
						}
					}
				}
		...
		// Try to close all open GST windows
		if (bOK)
		{
			BOOL GSTWindowClosed = FALSE;

			EnumWindows (CloseGSTWindowsEnum, (LPARAM) &GSTWindowClosed);
		...
		if (!bUpgrade)
		{
			...
			{
				MessageBoxW (hwndDlg, GetString ("CLOSE_GST_FIRST"), lpszTitle, MB_ICONSTOP);
				bOK = FALSE;
			}
		}
	...
	else
	{
		...
		// guaranteed that the user is installing GostCrypt for the first time now (we also cannot know if the user has already
		// installed and used GostCrypt on another system before).
...
BOOL UpgradeBootLoader (HWND hwndDlg)
{
	...
	try
	{
		...
		if (bootEnc.GetInstalledBootLoaderVersion() < VERSION_NUM)
		{
			...
			if (bootEnc.GetInstalledBootLoaderVersion() <= GST_RESCUE_DISK_UPGRADE_NOTICE_MAX_VERSION)
...
{
	char szLinkDir[GST_MAX_PATH];
	char szTmp2[GST_MAX_PATH];
	BOOL bSlash, bOK = FALSE;
	HRESULT hOle;
	int x;
	BOOL allUsers = FALSE;
	...
	// Start menu entries
	sprintf (szTmp2, "%s%s", szLinkDir, "\\GostCrypt.lnk");
	RemoveMessage (hwndDlg, szTmp2);
	if (StatDeleteFile (szTmp2) == FALSE)
		goto error;

	sprintf (szTmp2, "%s%s", szLinkDir, "\\GostCrypt Website.url");
	RemoveMessage (hwndDlg, szTmp2);
	if (StatDeleteFile (szTmp2) == FALSE)
		goto error;

	sprintf (szTmp2, "%s%s", szLinkDir, "\\Uninstall GostCrypt.lnk");
	RemoveMessage (hwndDlg, szTmp2);
	if (StatDeleteFile (szTmp2) == FALSE)
		goto error;
	
	sprintf (szTmp2, "%s%s", szLinkDir, "\\GostCrypt User's Guide.lnk");
	DeleteFile (szTmp2);
...
{
	char szLinkDir[GST_MAX_PATH], szDir[GST_MAX_PATH];
	char szTmp[GST_MAX_PATH], szTmp2[GST_MAX_PATH], szTmp3[GST_MAX_PATH];
	BOOL bSlash, bOK = FALSE;
	HRESULT hOle;
	int x;
	...
	if (bProgGroup)
	{
		...
		sprintf (szTmp, "%s%s", szDir, "GostCrypt.exe");
		sprintf (szTmp2, "%s%s", szLinkDir, "\\GostCrypt.lnk");
		...
		sprintf (szTmp, "%s%s", szDir, "GostCrypt Setup.exe");
		sprintf (szTmp2, "%s%s", szLinkDir, "\\Uninstall GostCrypt.lnk");
		strcpy (szTmp3, "/u");
...
static void SetSystemRestorePoint (HWND hwndDlg, BOOL finalize)
{
	...
	if (!finalize)
	{
		...
		strcpy (RestPtInfo.szDescription, bUninstall ? "GostCrypt uninstallation" : "GostCrypt installation");
...
void DoUninstall (void *arg)
{
	...
	else
	{
		...
		if (DoServiceUninstall (hwndDlg, "gostcrypt") == FALSE)
		{
			bOK = FALSE;
		}
		...
		else
		{
			...
			// Deprecated service
			DoServiceUninstall (hwndDlg, "GostCryptService");

			GetTempPath (sizeof (temp), temp);
			_snprintf (UninstallBatch, sizeof (UninstallBatch), "%s\\GostCrypt-Uninstall.bat", temp);
			...
			else
			{
				fprintf (f, ":loop\n"
					...
					"rmdir \"%s\"\n"
					"del \"%s\"",
					InstallationPath, "GostCrypt Setup.exe",
					InstallationPath, "GostCrypt Setup.exe",
					InstallationPath,
					UninstallBatch
					);
	...
	if (bOK)
		PostMessage (hwndDlg, GST_APPMSG_UNINSTALL_SUCCESS, 0, 0);
	else
...
void DoInstall (void *arg)
{
	...
	if (mkfulldir (InstallationPath, TRUE) != 0)
	{
		if (mkfulldir (InstallationPath, FALSE) != 0)
		{
			...
			MessageBoxW (hwndDlg, szTmp, lpszTitle, MB_ICONHAND);
			Error ("INSTALL_FAILED");
			PostMessage (MainDlg, GST_APPMSG_INSTALL_FAILURE, 0, 0);
			return;
		}
	}
	...
	if (bUpgrade
		&& (IsFileInUse (string (InstallationPath) + '\\' + GST_APP_NAME ".exe")
			|| IsFileInUse (string (InstallationPath) + '\\' + GST_APP_NAME " Format.exe")
			|| IsFileInUse (string (InstallationPath) + '\\' + GST_APP_NAME " Setup.exe")
			)
		)
	{
		NormalCursor ();
		Error ("CLOSE_GST_FIRST");
		PostMessage (MainDlg, GST_APPMSG_INSTALL_FAILURE, 0, 0);
		return;
	}
	...
	if (SystemEncryptionUpdate && InstalledVersion < 0x700)
	{
		...
		if (ReadDriverConfigurationFlags() & GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD_FOR_SYS_FAVORITES)
		{
	...
	if (bUpgrade && InstalledVersion < 0x700)
	{
		...
		{
			char *favoritesFilename = GetConfigPath (GST_APPD_FILENAME_FAVORITE_VOLUMES);
			DWORD size;
			...
			if (favoritesXml && size != 0)
			{
				...
				if (bMountFavoritesOnLogon)
				{
					...
					ReadRegistryString (regk, "GostCrypt", "", regVal, sizeof (regVal));

					if (strstr (regVal, "favorites"))
					{
						strcat_s (regVal, sizeof (regVal), " /a logon");
						WriteRegistryString (regk, "GostCrypt", regVal);
					}
				}
			}
	...
	strcat_s (path, sizeof (path), "\\GostCrypt Setup.exe");
	DeleteFile (path);

	if (UpdateProgressBarProc(63) && UnloadDriver && DoServiceUninstall (hwndDlg, "gostcrypt") == FALSE)
	{
		bOK = FALSE;
	}
	...
	{
		string sysFavorites = GetServiceConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES);
...
outcome:
	...
	if (bOK && !bUninstall && !bDowngrade && !bRepairMode && !bDevm)
	{
		if (!IsHiddenOSRunning())	// A hidden OS user should not see the post-install notes twice (on decoy OS and then on hidden OS).
		{
			if (bRestartRequired || SystemEncryptionUpdate)
			{
				...
				if (bUpgrade)
				{
					SavePostInstallTasksSettings (GST_POST_INSTALL_CFG_RELEASE_NOTES);
				}
				else if (bPossiblyFirstTimeInstall)
				{
					SavePostInstallTasksSettings (GST_POST_INSTALL_CFG_TUTORIAL);
				}
			}
			else
			{
	...
	PostMessage (MainDlg, bOK ? GST_APPMSG_INSTALL_SUCCESS : GST_APPMSG_INSTALL_FAILURE, 0, 0);
}
...
void SetInstallationPath (HWND hwndDlg)
{
	...
	// Determine if GostCrypt is already installed and try to determine its "Program Files" location
	...
	{
		/* Default 'UninstallString' registry strings written by past versions of GostCrypt:
		------------------------------------------------------------------------------------
		1.0		C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		1.0a	C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		2.0		C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		2.1		C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		2.1a	C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		3.0		C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		3.0a	C:\WINDOWS\GostCrypt Setup.exe /u			[optional]
		3.1		The UninstallString was NEVER written (fortunately, 3.1a replaced 3.1 after 2 weeks)
		3.1a	C:\WINDOWS\GostCrypt Setup.exe /u
		4.0		C:\WINDOWS\GostCrypt Setup.exe /u C:\Program Files\GostCrypt
		4.1		C:\WINDOWS\GostCrypt Setup.exe /u C:\Program Files\GostCrypt
		4.2		C:\WINDOWS\GostCrypt Setup.exe /u C:\Program Files\GostCrypt
		4.2a	C:\WINDOWS\GostCrypt Setup.exe /u C:\Program Files\GostCrypt
		4.3		"C:\Program Files\GostCrypt\GostCrypt Setup.exe" /u C:\Program Files\GostCrypt\
		4.3a	"C:\Program Files\GostCrypt\GostCrypt Setup.exe" /u C:\Program Files\GostCrypt\
		5.0+	"C:\Program Files\GostCrypt\GostCrypt Setup.exe" /u
		...
		*/
		...
		if (RegQueryValueEx (hkey, "UninstallString", 0, 0, (LPBYTE) &rv, &size) == ERROR_SUCCESS && strrchr (rv, '/'))
		{
			...
			// Cut and paste the location (path) where GostCrypt is installed to InstallationPath
			if (rv[0] == '"')
			{
				// 4.3 or later
			...
			else
			{
				...
				else
				{
					...
					// We know that GostCrypt is installed but don't know where. It's not safe to continue installing
	...
	if (bInstallPathDetermined)
	{
		...
		// Determine whether we were launched from the folder where GostCrypt is installed
		GetModuleFileName (NULL, mp, sizeof (mp));
		if (strncmp (InstallationPath, mp, min (strlen(InstallationPath), strlen(mp))) == 0)
		{
			// We were launched from the folder where GostCrypt is installed
	...
	else
	{
		...
		strncat (path, "\\GostCrypt\\", min (strlen("\\GostCrypt\\"), sizeof(path)-strlen(path)-1));
...
BOOL CALLBACK UninstallDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case GST_APPMSG_UNINSTALL_SUCCESS:
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpszCommandLine, int nCmdShow)
{
	...
	else
	{
		...
		if (!bUninstall)
		{
			...
			if (bChangeMode)
			{
				/* GostCrypt is already installed on this system and we were launched from the Program Files folder */
		...
		// System Restore
		SystemRestoreDll = LoadLibraryEx("srclient.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
