...
#define OutputPackageFile "GostCrypt Setup " VERSION_STRING ".exe"

#define MAG_START_MARKER	"GSTINSTRT"
...
char DestExtractPath [GST_MAX_PATH];
...
{
	MessageBox (NULL, msg, "GostCrypt", MB_ICONERROR | MB_SETFOREGROUND | MB_TOPMOST);
}


static void PkgWarning (char *msg)
{
	MessageBox (NULL, msg, "GostCrypt", MB_ICONWARNING | MB_SETFOREGROUND | MB_TOPMOST);
}


static void PkgInfo (char *msg)
{
	MessageBox (NULL, msg, "GostCrypt", MB_ICONINFORMATION | MB_SETFOREGROUND | MB_TOPMOST);
}
...
{
	int i, x;
	unsigned char inputFile [GST_MAX_PATH];
	unsigned char outputFile [GST_MAX_PATH];
	unsigned char szTmpFilePath [GST_MAX_PATH];
	...
	// Clone 'GostCrypt Setup.exe' to create the base of the new self-extracting archive

	if (!GSTCopyFile (inputFile, outputFile))
	{
		handleWin32Error (hwndDlg);
		PkgError ("Cannot copy 'GostCrypt Setup.exe' to the package");
		goto err;
	}
...
void __cdecl ExtractAllFilesThread (void *hwndDlg)
{
	...
	for (fileNo = 0; fileNo < NBR_COMPRESSED_FILES; fileNo++)
	{
		char fileName [GST_MAX_PATH] = {0};
		char filePath [GST_MAX_PATH] = {0};
...
eaf_end:
	...
	if (bSuccess)
		PostMessage (MainDlg, GST_APPMSG_EXTRACTION_SUCCESS, 0, 0);
	else
		PostMessage (MainDlg, GST_APPMSG_EXTRACTION_FAILURE, 0, 0);
}
