...
namespace GostCrypt
{
	...
	BOOL AddMountedVolumeToFavorites (HWND hwndDlg, int driveNo, bool systemFavorites)
	{
		...
		if (!DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop, sizeof (prop), &prop, sizeof (prop), &bytesReturned, NULL))
		{
		...
		if (IsVolumeDeviceHosted (favorite.Path.c_str()))
		{
			...
			wchar_t resolvedVolumeDevPath[GST_MAX_PATH];
			...
			if (find != INVALID_HANDLE_VALUE)
			{
				do
				{
					char findVolumeDevPath[GST_MAX_PATH];
			...
			case IDOK:
				...
				if (SystemFavoritesMode)
				{
					...
					if (BootEncObj.GetStatus().DriveMounted)
					{
						try
						{
							uint32 reqConfig = IsDlgButtonChecked (hwndDlg, IDC_FAVORITE_OPEN_EXPLORER_WIN_ON_MOUNT) ? GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD_FOR_SYS_FAVORITES : 0;
							if (reqConfig != (ReadDriverConfigurationFlags() & GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD_FOR_SYS_FAVORITES))
								BootEncObj.RegisterSystemFavoritesService (reqConfig ? TRUE : FALSE);

							SetDriverConfigurationFlag (GST_DRIVER_CONFIG_DISABLE_NONADMIN_SYS_FAVORITES_ACCESS, IsDlgButtonChecked (hwndDlg, IDC_FAVORITE_DISABLE_HOTKEY));
						}
						catch (Exception &e)
						{
							e.Show (hwndDlg);
	...
	static void FillFavoriteVolumesMenu ()
	{
		...
		foreach (const FavoriteVolume &favorite, FavoriteVolumes)
		{
			...
			AppendMenuW (FavoriteVolumesMenu, flags, GST_FAVORITE_MENU_CMD_ID_OFFSET + i++,
	...
	{
		favorites.clear();
		string favoritesFilePath = systemFavorites ? GetServiceConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES) : GetConfigPath (GST_APPD_FILENAME_FAVORITE_VOLUMES);

		if (systemFavorites && !IsAdmin() && !noUacElevation)
		{
			favoritesFilePath = GetConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES);

			try
			{
				BootEncryption bootEnc (MainDlg);
				bootEnc.CopyFileAdmin (GetServiceConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES).c_str(), favoritesFilePath.c_str());
			}
		...
		if (systemFavorites && !IsAdmin() && !noUacElevation)
			DeleteFile (GetConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES));
	...
	static bool SaveFavoriteVolumes (const vector <FavoriteVolume> &favorites, bool systemFavorites)
	{
		...
		f = fopen (GetConfigPath (systemFavorites ? GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES : GST_APPD_FILENAME_FAVORITE_VOLUMES), "w,ccs=UTF-8");
		if (f == NULL)
		{
		...
		if (!CheckFileStreamWriteErrors (f, systemFavorites ? GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES : GST_APPD_FILENAME_FAVORITE_VOLUMES))
		{
			fclose (f);
			return false;
		}
		...
		if (systemFavorites)
		{
			finally_do ({ remove (GetConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES)); });

			try
			{
				bootEnc.DeleteFileAdmin (GetServiceConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES).c_str());
			}
			...
			try
			{
				if (cnt != 0)
				{
					bootEnc.CopyFileAdmin (GetConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES), GetServiceConfigPath (GST_APPD_FILENAME_SYSTEM_FAVORITE_VOLUMES).c_str());
		...
		if (cnt == 0)
		{
			if (systemFavorites)
			{
				...
				remove (GetConfigPath (GST_APPD_FILENAME_FAVORITE_VOLUMES));
		}
	...
	static void SetControls (HWND hwndDlg, const FavoriteVolume &favorite, bool systemFavoritesMode, bool enable)
	{
		...
		if (systemFavoritesMode)
		{
			...
			// MOUNT_SYSTEM_FAVORITES_ON_BOOT
			CheckDlgButton (hwndDlg, IDC_FAVORITE_OPEN_EXPLORER_WIN_ON_MOUNT, (driverConfig & GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD_FOR_SYS_FAVORITES) ? BST_CHECKED : BST_UNCHECKED);

			// DISABLE_NONADMIN_SYS_FAVORITES_ACCESS
			CheckDlgButton (hwndDlg, IDC_FAVORITE_DISABLE_HOTKEY, (driverConfig & GST_DRIVER_CONFIG_DISABLE_NONADMIN_SYS_FAVORITES_ACCESS) ? BST_CHECKED : BST_UNCHECKED);
		}
		else
		{
