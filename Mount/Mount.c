...
{
	GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_NONE = 0,
	GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_COMPACT,
	GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_DISABLED
};
...
char szFileName[GST_MAX_PATH+1];		/* Volume to mount */
...
BOOL bHiddenSysLeakProtNotifiedDuringSession = FALSE;	/* TRUE if the user has been notified during the session that unencrypted filesystems and non-hidden GostCrypt volumes are mounted as read-only under hidden OS. */
...
int HiddenSysLeakProtectionNotificationStatus = GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_NONE;
...
void EndMainDlg (HWND hwndDlg)
{
	...
	{
		DWORD dwResult;
		DeviceIoControl (hDriver, GST_IOCTL_WIPE_PASSWORD_CACHE, NULL, 0, NULL, 0, &dwResult, NULL);
	}
...
static void InitMainDialog (HWND hwndDlg)
{
	...
	SetWindowLongPtr (hwndDlg, DWLP_USER, (LONG_PTR) (IsAdmin() ? GST_MAIN_WINDOW_FLAG_ADMIN_PRIVILEGES : 0));

	DragAcceptFiles (hwndDlg, TRUE);

	SendMessage (GetDlgItem (hwndDlg, IDC_VOLUME), CB_LIMITTEXT, GST_MAX_PATH, 0);
...
void EnableDisableButtons (HWND hwndDlg)
{
	...
	switch (x)
	{
	case GST_MLIST_ITEM_NONSYS_VOL:
		{
	...
	case GST_MLIST_ITEM_SYS_PARTITION:
	case GST_MLIST_ITEM_SYS_DRIVE:
...
{
	EnableHwEncryption ((ReadDriverConfigurationFlags() & GST_DRIVER_CONFIG_DISABLE_HARDWARE_ENCRYPTION) ? FALSE : TRUE);

	WipeAlgorithmId savedWipeAlgorithm = GST_WIPE_NONE;
	...
	if (IsHiddenOSRunning())
		HiddenSysLeakProtectionNotificationStatus =	ConfigReadInt ("HiddenSystemLeakProtNotifStatus", GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_NONE);
...
{
	/* If you update this function, revise SysEncryptionOrDecryptionRequired() in Gstformat.c as well. */
...
{
	/* If you update this function, revise SysDriveOrPartitionFullyEncrypted() in Gstformat.c as well. */
...
// Returns TRUE if the system partition/drive is being filtered by the GostCrypt driver and the key data
...
BOOL WholeSysDriveEncryption (BOOL bSilent)
{
	try
	{
		...
		return (BootEncStatus.ConfiguredEncryptedAreaStart == GST_BOOT_LOADER_AREA_SIZE
...
{
	BOOL tmpbDevice;
	char szDevicePath [GST_MAX_PATH+1];
	char szDiskFile [GST_MAX_PATH+1];
	...
	if (strlen (devicePath) < 2)
	{
		...
		if (LOWORD (GetSelectedLong (GetDlgItem (MainDlg, IDC_DRIVELIST))) != GST_MLIST_ITEM_FREE)
		{
			if (!quiet)
	...
	try
	{
		...
		if (BootEncStatus.DriveMounted)
		{
			int retCode = 0;
			int driveNo;
			char parentDrivePath [GST_MAX_PATH+1];
...
// Returns TRUE if the host drive of the specified partition contains a portion of the GostCrypt Boot Loader
...
BOOL GSTBootLoaderOnInactiveSysEncDrive (void) 
{
	try
	{
		int driveNo;
...
void LoadDriveLetters (HWND hTree, int drive)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, &driver,
	...
	if (bWholeSysDriveEncryption)
	{
...

		{
			...
			listItem.lParam = MAKELONG (GST_MLIST_ITEM_SYS_DRIVE, ENC_SYSDRIVE_PSEUDO_DRIVE_LETTER);	
	...
	for (i = 2; i < 26; i++)
	{
			...
			|| bSysEncPartition)
		{
			...
			listItem.lParam = MAKELONG (
				bSysEncPartition ? GST_MLIST_ITEM_SYS_PARTITION : GST_MLIST_ITEM_NONSYS_VOL, 
				i + 'A');
		...
		else
		{
			...
			if (!(dwUsedDrives & 1 << i))
			{
				...
				listItem.iItem = item++;  
				listItem.pszText = szDriveLetters[i];
				listItem.lParam = MAKELONG (GST_MLIST_ITEM_FREE, i + 'A');
...
BOOL CALLBACK PasswordDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			if (bPrebootPasswordDlgMode)
			{
				SendMessage (hwndDlg, GST_APPMSG_PREBOOT_PASSWORD_MODE, 0, 0);
			}
	...
	case GST_APPMSG_PREBOOT_PASSWORD_MODE:
		{
			ToBootPwdField (hwndDlg, IDC_PASSWORD);
...
int GetCipherBlockSizeByDriveNo (int nDosDriveNo)
{
	...
	if (DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop, sizeof (prop), &prop, sizeof (prop), &dwResult, NULL))
	{
	...
	case WM_INITDIALOG:
		{
			...
			{
				if (!DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop, sizeof (prop), &prop, sizeof (prop), &dwResult, NULL) || dwResult == 0)
					return 0;
			}
			...
			if (bSysEnc)
			{
				// GostCrypt Boot Loader version
				ListItemAddW (list, i, GetString ("GST_BOOT_LOADER_VERSION"));
...
BOOL CALLBACK TravelerDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if (lw == IDC_CREATE)
		{
			...
			sprintf (dstPath, "%s\\GostCrypt", dstDir);
			CreateDirectory (dstPath, NULL);

			// Main app
			sprintf (srcPath, "%s\\GostCrypt.exe", appDir);
			sprintf (dstPath, "%s\\GostCrypt\\GostCrypt.exe", dstDir);
			if (!GSTCopyFile (srcPath, dstPath))
			{
				handleWin32Error (hwndDlg);
				goto stop;
			}
			...
			if (copyWizard)
			{
				sprintf (srcPath, "%s\\GostCrypt Format.exe", appDir);
				sprintf (dstPath, "%s\\GostCrypt\\GostCrypt Format.exe", dstDir);
				if (!GSTCopyFile (srcPath, dstPath))
				{
					handleWin32Error (hwndDlg);
					goto stop;
			...
			// Driver
			sprintf (srcPath, "%s\\gostcrypt.sys", appDir);
			sprintf (dstPath, "%s\\GostCrypt\\gostcrypt.sys", dstDir);
			if (!GSTCopyFile (srcPath, dstPath))
			{
				handleWin32Error (hwndDlg);
				goto stop;
			...
			// Driver x64
			sprintf (srcPath, "%s\\gostcrypt-x64.sys", appDir);
			sprintf (dstPath, "%s\\GostCrypt\\gostcrypt-x64.sys", dstDir);
			if (!GSTCopyFile (srcPath, dstPath))
			{
				handleWin32Error (hwndDlg);
				goto stop;
			...
			if (GetPreferredLangId () && strcmp (GetPreferredLangId (), "en") != 0)
			{
				...
				sprintf (dstPath, "%s\\GostCrypt\\Language.%s.xml", dstDir, GetPreferredLangId ());
				GSTCopyFile (srcPath, dstPath);
			}
			...
			if (bAutoRun)
			{
				...
				sprintf (autoMount, "GostCrypt\\GostCrypt.exe /q background%s%s%s%s /m rm /v %s",
					drive > 0 ? driveLetter : "",
					bExplore ? " /e" : "",
					bCacheInDriver ? " /c y" : "",
					bMountReadOnly ? " /m ro" : "",
				...
				fwprintf (af, L"[autorun]\nlabel=%s\nicon=GostCrypt\\GostCrypt.exe\n", GetString ("GST_TRAVELER_DISK"));
				fwprintf (af, L"action=%s\n", bAutoMount ? GetString ("MOUNT_GST_VOLUME") : GetString ("IDC_PREF_LOGON_START"));
				fwprintf (af, L"open=%hs\n", bAutoMount ? autoMount : "GostCrypt\\GostCrypt.exe");
				fwprintf (af, L"shell\\start=%s\nshell\\start\\command=GostCrypt\\GostCrypt.exe\n", GetString ("IDC_PREF_LOGON_START"));
				fwprintf (af, L"shell\\dismount=%s\nshell\\dismount\\command=GostCrypt\\GostCrypt.exe /q /d\n", GetString ("DISMOUNT_ALL_GST_VOLUMES"));
...
retry:
	...
	DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, &mountList, sizeof (mountList), &mountList, sizeof (mountList), &dwResult, NULL);
	...
	do
	{
		bResult = DeviceIoControl (hDriver, GST_IOCTL_DISMOUNT_ALL_VOLUMES, &unmount,
	...
	if (unmount.nReturnCode != 0)
	{
		...
		if (unmount.nReturnCode == ERR_FILES_OPEN)
		{
			...
			if (IsOSAtLeast (WIN_7))
			{
				...
				DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, NULL, 0, &mountList, sizeof (mountList), &dwResult, NULL);
		...
		foreach (const HostDevice &drive, devices)
		{
			...
			foreach (const HostDevice &device, partitions)
			{
				...
				{	
					...
					{
						if(LOWORD (GetItemLong (driveList, selDrive)) != GST_MLIST_ITEM_FREE)
						{
							selDrive++;
							continue;
						}
...
static void EncryptSystemDevice (void)
{
		...
		&& !SysEncryptionOrDecryptionRequired ())
	{
		...
		if (!MutexExistsOnSystem (GST_MUTEX_NAME_SYSENC))	// If no instance of the wizard is currently taking care of system encryption
		{
...
void CreateRescueDisk (void)
{
	...
	if (CreateSysEncMutex ())	// If no instance of the wizard is currently taking care of system encryption
	{
		...
		{
			wchar_t szTmp [8096];
			char szRescueDiskISO [GST_MAX_PATH+1];
			...
			if (!BrowseFilesInDir (MainDlg, "OPEN_TITLE", initialDir, szRescueDiskISO, FALSE, TRUE, NULL, L"GostCrypt Rescue Disk.iso", L"iso"))
			{		
				CloseSysEncMutex ();
				return;
			}
...
static void WipeCache (HWND hwndDlg, BOOL silent)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_WIPE_PASSWORD_CACHE, NULL, 0, NULL, 0, &dwResult, NULL);
	if (hwndDlg == NULL)
		return;
...
static BOOL CheckMountList ()
{
		...
		&& TaskBarIconMutex != NULL)
	{
		...
		TaskBarIconChange (MainDlg, current.ulMountedDrives != 0 && bUseDifferentTrayIconIfVolMounted ? IDI_GOSTCRYPT_MOUNTED_ICON : IDI_GOSTCRYPT_ICON);
	}
	...
	try
	{
		...
		if (IsHiddenOSRunning())
		{
				...
				&& !bHiddenSysLeakProtNotifiedDuringSession)
			{
				...
				{
				case GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_COMPACT:
					{
						...
						case 2:
							...
							HiddenSysLeakProtectionNotificationStatus = GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_DISABLED;
				...
				case GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_DISABLED:
					// NOP
					break;

				case GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_NONE:
				default:
					{
						...
						HiddenSysLeakProtectionNotificationStatus = GST_HIDDEN_OS_READ_ONLY_NOTIF_MODE_COMPACT;
						SaveSettings (MainDlg);
					}
					break;
				}
	...
	case WM_INITDIALOG:
		{
			...
			if (IsGostCryptInstallerRunning())
				AbortProcess ("GST_INSTALLER_IS_RUNNING");
			...
			try
			{
				if (IsHiddenOSRunning())
				{
					if (BootEncObj->GetInstalledBootLoaderVersion() > VERSION_NUM)
						Warning ("UPDATE_GST_IN_HIDDEN_OS_TOO");
				}
			...
			if (cmdUnmountDrive > 0)
			{
				...
				if (DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, NULL, 0, &mountList, sizeof (mountList), &bytesReturned, NULL)
			...
			{
				HWND h = hwndDlg;
				EnumWindows (FindGSTWindowEnum, (LPARAM) &h);

				if (h != hwndDlg
					&& (!IsAdmin() || (GetWindowLongPtr (h, DWLP_USER) & GST_MAIN_WINDOW_FLAG_ADMIN_PRIVILEGES) != 0))
				{
					if (CmdLineVolumeSpecified)
					{
						COPYDATASTRUCT cd;
					...
					SendMessage (h, GST_APPMSG_MOUNT_SHOW_WINDOW, 0, 0);
			...
			if (!RegisterAllHotkeys (hwndDlg, Hotkeys)
				&& TaskBarIconMutex != NULL)	// Warn only if we are the first instance of GostCrypt
				...
				&& IsOSAtLeast (WIN_7))
			{
				...
				int maxYear = GST_RELEASE_DATE_YEAR;
				int maxMonth = GST_RELEASE_DATE_MONTH + detectionPeriodInMonthsSinceReleaseDate;
				if (maxMonth > 12)
				{
					++maxYear;
					maxMonth -= 12;
				...
				if (systemTime.wYear >= GST_RELEASE_DATE_YEAR
					&& !(systemTime.wYear == GST_RELEASE_DATE_YEAR && systemTime.wMonth < GST_RELEASE_DATE_MONTH)
	...
	case WM_TIMER:
		{
			...
			if (DeviceIoControl (hDriver, GST_IOCTL_GET_WARNING_FLAGS, NULL, 0, &warnings, sizeof (warnings), &bytesOut, NULL))
			{
...
#error TIMER_INTERVAL_MAIN != 500
				...
				if ((++favoritesAutoMountTimerDivisor & 1) && !FavoritesOnArrivalMountRequired.empty())
				{
					...
					foreach (FavoriteVolume favorite, FavoritesOnArrivalMountRequired)
					{
						if (!favorite.VolumePathId.empty())
						{
							...
							if (QueryDosDevice (favorite.VolumePathId.substr (4, favorite.VolumePathId.size() - 5).c_str(), volDevPath, GST_MAX_PATH) == 0)
								continue;
						...
						deleted ? favorite : ++favorite)
					{
						...
						if (favorite->VolumePathId.size() > 5
							&& QueryDosDevice (favorite->VolumePathId.substr (4, favorite->VolumePathId.size() - 5).c_str(), volDevPath, GST_MAX_PATH) != 0)
						{
							continue;
						}
			...
			case WM_RBUTTONUP:
				{
					...
					if (MainWindowHidden)
					{
						AppendMenuW (popup, MF_STRING, IDM_SHOW_HIDE, GetString ("SHOW_GST"));
						AppendMenu (popup, MF_SEPARATOR, 0, NULL);
					}
						...
						&& GetDriverRefCount () < 2)))
					{
						AppendMenuW (popup, MF_STRING, IDM_SHOW_HIDE, GetString ("HIDE_GST"));
						AppendMenu (popup, MF_SEPARATOR, 0, NULL);
					}
					...
					else if (sel == IDCANCEL)
					{
						...
						{
							// Close all other GST windows
							EnumWindows (CloseGSTWindowsEnum, 0);
	...
	case GST_APPMSG_CLOSE_BKG_TASK:
		if (TaskBarIconMutex != NULL)
			TaskBarIconRemove (hwndDlg);

		return 1;

	case GST_APPMSG_SYSENC_CONFIG_UPDATE:
		LoadSysEncSettings (hwndDlg);

		// The wizard added GostCrypt.exe to the system startup sequence or performed other operations that 
	...
	case WM_NOTIFY:
		...
		if(wParam == IDC_DRIVELIST)
		{
			...
			if (((LPNMHDR) lParam)->code == LVN_ITEMACTIVATE)
			{
				...
				if (LOWORD(state) == GST_MLIST_ITEM_NONSYS_VOL || LOWORD(state) == GST_MLIST_ITEM_SYS_PARTITION)
				{
				...
				else if (LOWORD (GetSelectedLong (GetDlgItem (hwndDlg, IDC_DRIVELIST))) == GST_MLIST_ITEM_FREE)
				{
					mountOptions = defaultMountOptions;
					bPrebootPasswordDlgMode = FALSE;
					...
					case IDPM_ADD_TO_SYSTEM_FAVORITES:
						{
							...
							if (LOWORD (selectedDrive) == GST_MLIST_ITEM_NONSYS_VOL)
	...
	case WM_COMMAND:
		...
		if (lw == IDOK && LOWORD (GetSelectedLong (GetDlgItem (hwndDlg, IDC_DRIVELIST))) == GST_MLIST_ITEM_NONSYS_VOL
			|| lw == IDM_UNMOUNT_VOLUME)
		{
			if (lw == IDM_UNMOUNT_VOLUME && LOWORD (GetSelectedLong (GetDlgItem (hwndDlg, IDC_DRIVELIST))) != GST_MLIST_ITEM_NONSYS_VOL)
			{
		...
		{
			MessageBoxW (hwndDlg, GetString ("SELECT_FREE_DRIVE"), L"GostCrypt", MB_ICONEXCLAMATION);
			return 1;
		}
		...
		if (lw == IDC_VOLUME_TOOLS)
		{
			...
			int menuItem;
			char volPath[GST_MAX_PATH];		/* Volume to mount */
			HMENU popup = CreatePopupMenu ();
			RECT rect;
		...
		if (lw == IDM_LICENSE)
		{
			TextInfoDialogBox (GST_TBXID_LEGAL_NOTICES);
			return 1;
		}
		...
		{
			//Applink ("faq", TRUE, "");
			Extlink("http://www.gostcrypt.org/redirect.php?a=4");
			return 1;
		}
		else if (lw == IDM_GST_DOWNLOADS)
		{
		...
		{
			//Applink ("news", TRUE, "");
			Extlink("http://www.gostcrypt.org/redirect.php?a=5");
			return 1;
		}
		else if (lw == IDM_VERSION_HISTORY)
		{
			//Applink ("history", TRUE, "");
			Extlink("http://www.gostcrypt.org/redirect.php?a=6");
			return 1;
		}
		...
		else if (lw == IDM_CONTACT)
		{
			...
			Extlink("http://www.gostcrypt.org/redirect.php?a=7");
			return 1;
		}
		...
		if (lw == IDM_ADD_VOLUME_TO_FAVORITES || lw == IDM_ADD_VOLUME_TO_SYSTEM_FAVORITES)
		{
			...
			char volPathLower[GST_MAX_PATH];
			wchar_t volPathLowerW[GST_MAX_PATH];
		...
		if (lw == IDC_VOLUME && hw == CBN_SELCHANGE)
		{
			...
			PostMessage (hwndDlg, GST_APPMSG_MOUNT_ENABLE_DISABLE_CONTROLS, 0, 0);
			return 1;
		}
		...
		if (lw >= GST_FAVORITE_MENU_CMD_ID_OFFSET && lw < GST_FAVORITE_MENU_CMD_ID_OFFSET_END)
		{
			size_t favoriteIndex = lw - GST_FAVORITE_MENU_CMD_ID_OFFSET;
	...
	case GST_APPMSG_MOUNT_ENABLE_DISABLE_CONTROLS:
		EnableDisableButtons (hwndDlg);
		return 1;

	case GST_APPMSG_MOUNT_SHOW_WINDOW:
		MainWindowHidden = FALSE;
		ShowWindow (hwndDlg, SW_SHOW);
		ShowWindow (hwndDlg, SW_RESTORE);
		return 1;
...
{ 
	HANDLE eventSource = RegisterEventSource (NULL, GST_SYSTEM_FAVORITES_SERVICE_NAME);

	if (eventSource)
	{
		LPCTSTR strings[] = { GST_SYSTEM_FAVORITES_SERVICE_NAME, errorMessage.c_str() };
...
static VOID WINAPI SystemFavoritesServiceMain (DWORD argc, LPTSTR *argv)
{
	...
	SystemFavoritesServiceStatusHandle = RegisterServiceCtrlHandler (GST_SYSTEM_FAVORITES_SERVICE_NAME, SystemFavoritesServiceCtrlHandler);
...
static BOOL StartSystemFavoritesService ()
{
	...
	serviceTable[0].lpServiceName = GST_SYSTEM_FAVORITES_SERVICE_NAME;
	...
	if (!(ReadDriverConfigurationFlags() & GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD))
		WipeCache (NULL, TRUE);
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpszCommandLine, int nCmdShow)
{
	...
	if (argv && argc == 2 && SingleStringToWide (GST_SYSTEM_FAVORITES_SERVICE_CMDLINE_OPTION) == argv[1])
...
{ 
	...
	TaskBarIconMutex = CreateMutex (NULL, TRUE, "GostCryptTaskBarIcon");
	...
	tnid.uCallbackMessage = GST_APPMSG_TASKBAR_ICON; 
	tnid.hIcon = (HICON) LoadImage (hInst, MAKEINTRESOURCE (IDI_GOSTCRYPT_ICON), 
		IMAGE_ICON, 
		ScreenDPI >= 120 ? 0 : 16, 
		ScreenDPI >= 120 ? 0 : 16,
...
void DismountIdleVolumes ()
{
	...
	for (i = 0; i < 26; i++)
	{
		if (LastKnownMountList.ulMountedDrives & (1 << i))
		{
			...
			bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop,
...
skipMount:
			...
			{
				DWORD bytesOut;
				DeviceIoControl (hDriver, GST_IOCTL_SET_SYSTEM_FAVORITE_VOLUME_DIRTY, NULL, 0, NULL, 0, &bytesOut, NULL);
...
static void SaveDefaultKeyFilesParam (void)
{
	if (defaultKeyFilesParam.FirstKeyFile == NULL)
	{
		...
		remove (GetConfigPath (GST_APPD_FILENAME_DEFAULT_KEYFILES));
	}
	else
	{
		FILE *f;
		...
		f = fopen (GetConfigPath (GST_APPD_FILENAME_DEFAULT_KEYFILES), "w");
		if (f == NULL)
		{
		...
		CheckFileStreamWriteErrors (f, GST_APPD_FILENAME_DEFAULT_KEYFILES);
		fclose (f);
		return;
	}
}
...
static void HandleHotKey (HWND hwndDlg, WPARAM wParam)
{
	...
	case HK_FORCE_DISMOUNT_ALL_AND_WIPE:
		...
		success &= DeviceIoControl (hDriver, GST_IOCTL_WIPE_PASSWORD_CACHE, NULL, 0, NULL, 0, &dwResult, NULL);
		if (success)
		{
			if (bDisplayBalloonOnSuccessfulHkDismount)
...
int BackupVolumeHeader (HWND hwndDlg, BOOL bRequireConfirmation, char *lpszVolume)
{
	...
	for (int type = GST_VOLUME_TYPE_NORMAL; type <= GST_VOLUME_TYPE_HIDDEN; ++type)
	{
		OpenVolumeContext *askVol = (type == GST_VOLUME_TYPE_HIDDEN ? &hiddenVolume : &volume);
		Password *askPassword = (type == GST_VOLUME_TYPE_HIDDEN ? &hiddenVolPassword : &VolumePassword);

		while (TRUE)
		{
			if (!AskVolumePassword (hwndDlg, askPassword, type == GST_VOLUME_TYPE_HIDDEN ? "ENTER_HIDDEN_VOL_PASSWORD" : "ENTER_NORMAL_VOL_PASSWORD", FALSE))
			{
				nStatus = ERR_SUCCESS;
				goto ret;
			}
			...
			{
				if ((type == GST_VOLUME_TYPE_NORMAL && askVol->CryptoInfo->hiddenVolume)
					|| (type == GST_VOLUME_TYPE_HIDDEN && !askVol->CryptoInfo->hiddenVolume))
				{
					CloseVolume (askVol);
				...
				if (type == GST_VOLUME_TYPE_NORMAL)
				{
...
noHidden:
	...
	byte backup[GST_VOLUME_HEADER_GROUP_SIZE];

	bool legacyVolume = volume.CryptoInfo->LegacyVolume ? true : false;
	int backupFileSize = legacyVolume ? GST_VOLUME_HEADER_SIZE_LEGACY * 2 : GST_VOLUME_HEADER_GROUP_SIZE;
	...
	{
		nStatus = ReEncryptVolumeHeader ((char *) backup + (legacyVolume ? GST_VOLUME_HEADER_SIZE_LEGACY : GST_VOLUME_HEADER_SIZE),
...
int RestoreVolumeHeader (HWND hwndDlg, char *lpszVolume)
{
	...
	if (restoreInternalBackup)
	{
		...
		headerOffset.QuadPart = volume.CryptoInfo->hiddenVolume ? GST_HIDDEN_VOLUME_HEADER_OFFSET : GST_VOLUME_HEADER_OFFSET;
	...
	else
	{
		...
		char buffer[GST_VOLUME_HEADER_GROUP_SIZE];
		...
		{
		case GST_VOLUME_HEADER_GROUP_SIZE:
			legacyBackup = false;
			break;

		case GST_VOLUME_HEADER_SIZE_LEGACY * 2:
			legacyBackup = true;
			break;
		...
		while (TRUE)
		{
			...
			for (int type = GST_VOLUME_TYPE_NORMAL; type <= GST_VOLUME_TYPE_HIDDEN; ++type)
			{
				if (type == GST_VOLUME_TYPE_HIDDEN)
					headerOffsetBackupFile += (legacyBackup ? GST_VOLUME_HEADER_SIZE_LEGACY : GST_VOLUME_HEADER_SIZE);
		...
		if (legacyBackup)
		{
			headerOffset.QuadPart = hiddenVol ? hostSize - GST_HIDDEN_VOLUME_HEADER_OFFSET_LEGACY : GST_VOLUME_HEADER_OFFSET;
		}
		else
		{
			headerOffset.QuadPart = hiddenVol ? GST_HIDDEN_VOLUME_HEADER_OFFSET : GST_VOLUME_HEADER_OFFSET;
			headerBackupOffset.QuadPart = hiddenVol ? hostSize - GST_VOLUME_HEADER_SIZE : hostSize - GST_VOLUME_HEADER_GROUP_SIZE;
		}
		...
		case IDOK:
			{
				...
				try
				{
					...
					DWORD bytesReturned;
					if (!DeviceIoControl (hDriver, GST_IOCTL_REREAD_DRIVER_CONFIG, NULL, 0, NULL, 0, &bytesReturned, NULL))
						handleWin32Error (hwndDlg);
					...
					{
						BootEncObj->WriteLocalMachineRegistryDwordValue ("SYSTEM\\CurrentControlSet\\Services\\gostcrypt", GST_ENCRYPTION_FREE_CPU_COUNT_REG_VALUE_NAME, cpuFreeCount);
	...
	case WM_INITDIALOG:
		{
			...
			try
			{
				...
				SendMessage (GetDlgItem (hwndDlg, IDC_CUSTOM_BOOT_LOADER_MESSAGE), EM_LIMITTEXT, GST_BOOT_SECTOR_USER_MESSAGE_MAX_LENGTH, 0);
				SetDlgItemText (hwndDlg, IDC_CUSTOM_BOOT_LOADER_MESSAGE, customUserMessage.c_str());

				CheckDlgButton (hwndDlg, IDC_DISABLE_BOOT_LOADER_OUTPUT, (userConfig & GST_BOOT_USER_CFG_FLAG_SILENT_MODE) ? BST_CHECKED : BST_UNCHECKED);
				CheckDlgButton (hwndDlg, IDC_ALLOW_ESC_PBA_BYPASS, (userConfig & GST_BOOT_USER_CFG_FLAG_DISABLE_ESC) ? BST_UNCHECKED : BST_CHECKED);
				CheckDlgButton (hwndDlg, IDC_BOOT_LOADER_CACHE_PASSWORD, (driverConfig & GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD) ? BST_CHECKED : BST_UNCHECKED);
		...
		case IDOK:
			{
				...
				if (IsDlgButtonChecked (hwndDlg, IDC_DISABLE_BOOT_LOADER_OUTPUT))
					userConfig |= GST_BOOT_USER_CFG_FLAG_SILENT_MODE;
				else
					userConfig &= ~GST_BOOT_USER_CFG_FLAG_SILENT_MODE;

				if (!IsDlgButtonChecked (hwndDlg, IDC_ALLOW_ESC_PBA_BYPASS))
					userConfig |= GST_BOOT_USER_CFG_FLAG_DISABLE_ESC;
				else
					userConfig &= ~GST_BOOT_USER_CFG_FLAG_DISABLE_ESC;
				...
				{
					BootEncObj->WriteBootSectorUserConfig (userConfig, customUserMessage);
					SetDriverConfigurationFlag (GST_DRIVER_CONFIG_CACHE_BOOT_PASSWORD, IsDlgButtonChecked (hwndDlg, IDC_BOOT_LOADER_CACHE_PASSWORD));
				}
				catch (Exception &e)
				{
					e.Show (hwndDlg);
...
void AnalyzeKernelMiniDump (HWND hwndDlg)
{
	...
	if (!IsApplicationInstalled (Is64BitOs() ? "Debugging Tools for Windows (x64)" : "Debugging Tools for Windows (x86)"))
	{
		...
		if (!CreateProcess (NULL, (LPSTR) (string ("msiexec.exe /qb /i " GST_APPLINK "&dest=ms-debug-tools-x") + (Is64BitOs() ? "64" : "86")).c_str(),
	...
	bool otherDriver = (StringToUpperCase (output).find (StringToUpperCase (GST_APP_NAME)) == string::npos);
	...
	sprintf (url, GST_APPLINK_SECURE "&dest=syserr-report&os=%s&osver=%d.%d.%d&arch=%s&err=%I64x&arg1=%I64x&arg2=%I64x&arg3=%I64x&arg4=%I64x&flag=%s&drv=%s",
	...
	if (!imageName.empty() && StringToUpperCase (imageName) != StringToUpperCase (GST_APP_NAME) + ".SYS")
	{
	...
	if (otherDriver)
	{
		msg += GetString ("SYSTEM_CRASH_NO_GOSTCRYPT");
		msg += L"\n\n";
	}
