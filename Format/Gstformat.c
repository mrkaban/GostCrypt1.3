...
char szFileName[GST_MAX_PATH+1];	/* The file selected by the user */
char szDiskFile[GST_MAX_PATH+1];	/* Fully qualified name derived from szFileName */
char szRescueDiskISO[GST_MAX_PATH+1];	/* The filename and path to the Rescue Disk ISO file to be burned (for boot encryption) */
...
static BOOL CALLBACK BroadcastSysEncCfgUpdateCallb (HWND hwnd, LPARAM lParam)
{
	if (GetWindowLongPtr (hwnd, GWLP_USERDATA) == (LONG_PTR) 'TRUE')
	{
		...
		{
			PostMessage (hwnd, GST_APPMSG_SYSENC_CONFIG_UPDATE, 0, 0);
		}
	}
	return TRUE;
}
...
static BOOL SaveSysEncSettings (HWND hwndDlg)
{
	...
	{
		if (remove (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION)) != 0)
		{
	...
	f = fopen (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION), "w");
	if (f == NULL)
	{
		Error ("CANNOT_SAVE_SYS_ENCRYPTION_SETTINGS");
...
{
	unsigned int phase = GST_HIDDEN_OS_CREATION_PHASE_NONE;
...
static BOOL ChangeSystemEncryptionStatus (int newStatus)
{
	...
	{
		if (DetermineHiddenOSCreationPhase() != GST_HIDDEN_OS_CREATION_PHASE_NONE
			&& !ChangeHiddenOSCreationPhase (GST_HIDDEN_OS_CREATION_PHASE_NONE))
			return FALSE;
...
BOOL SwitchWizardToSysEncMode (void)
{
	...
	// From now on, we should be the only instance of the GST wizard allowed to deal with system encryption
	if (!CreateSysEncMutex ())
	{
...
// decrypt the system partition/drive and to uninstall GostCrypt). The function removes the system encryption key 
// data ("volume header"), the GostCrypt boot loader, restores the original system loader (if available),
...
void ComboSelChangeEA (HWND hwndDlg)
{
	...
	else
	{
		...
		if (strcmp (name, "GOST 28147-89") == 0)
		{
			...
			SetWindowTextW (GetDlgItem (hwndDlg, IDC_BOX_HELP), GetString ("GOST_HELP"));
		}
...
static void VerifySizeAndUpdate (HWND hwndDlg, BOOL bUpdate)
{
	...
	if (bEnable)
	{
		if (lTmp * i < (bHiddenVolHost ? GST_MIN_HIDDEN_VOLUME_HOST_SIZE : (bHiddenVol ? GST_MIN_HIDDEN_VOLUME_SIZE : GST_MIN_VOLUME_SIZE)))
			bEnable = FALSE;
		...
		{
			if (lTmp * i > (bHiddenVolHost ? GST_MAX_HIDDEN_VOLUME_HOST_SIZE : GST_MAX_VOLUME_SIZE))
				bEnable = FALSE;
		}
	}
...
static void __cdecl volTransformThreadFunction (void *hwndDlgArg)
{
	...
	volParams->headerFlags = CreatingHiddenSysVol() ? GST_HEADER_FLAG_ENCRYPTED_SYSTEM : 0;
	...
	else
	{
		...
		nStatus = GSTFormatVolume (volParams);
	}
	...
	if (nStatus != ERR_USER_ABORT)
	{
		...
		else
		{
			...
			PostMessage (hwndDlg, bInPlaceEncNonSys ? GST_APPMSG_NONSYS_INPLACE_ENC_FINISHED : GST_APPMSG_FORMAT_FINISHED, 0, 0);
...
cancel:
	...
	PostMessage (hwndDlg, GST_APPMSG_VOL_TRANSFORM_THREAD_ENDED, 0, 0);
...
static BOOL FileSize4GBLimitQuestionNeeded (void)
{
	...
	return (dataAreaSize > 4 * BYTES_PER_GB + GST_MIN_FAT_FS_SIZE
		&& dataAreaSize <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize());
}
...
BOOL CALLBACK PageDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		case VOLUME_LOCATION_PAGE:
			{
				...
				SendMessage (GetDlgItem (hwndDlg, IDC_COMBO_BOX), CB_LIMITTEXT, GST_MAX_PATH, 0);
		...
		case HIDDEN_VOL_PRE_CIPHER_PAGE:
			{
				...
				if (bHiddenOS)
				{
					...
					if (nMaximumHiddenVolSize - GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE_HIGH < GetSystemPartitionSize())
					{
					...
					else
					{
						...
						nVolumeSize = GetSystemPartitionSize() + GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE_HIGH;	
		...
		case NONSYS_INPLACE_ENC_WIPE_MODE_PAGE:
			{
				if (nWipeMode == GST_WIPE_1_RAND)
					nWipeMode = GST_WIPE_NONE;
		...
		case SYSENC_ENCRYPTION_PAGE:
			...
			if (CreateSysEncMutex ())
			{
				...
				PostMessage (hwndDlg, GST_APPMSG_PERFORM_POST_SYSENC_WMINIT_TASKS, 0, 0);
			}
			else
			{
				Error ("SYSTEM_ENCRYPTION_IN_PROGRESS_ELSEWHERE");
		...
		case FORMAT_PAGE:
			{
				...
				for (int i = 1; i <= 128; i *= 2)
				{
					...
					if (size > GST_MAX_FAT_CLUSTER_SIZE)
						break;
				...
				if (!CreatingHiddenSysVol())	
				{
					if (dataAreaSize >= GST_MIN_NTFS_FS_SIZE && dataAreaSize <= GST_MAX_NTFS_FS_SIZE)
					{
						AddComboPair (GetDlgItem (hwndDlg, IDC_FILESYS), "NTFS", FILESYS_NTFS);
						bNTFSallowed = TRUE;
					}

					if (dataAreaSize >= GST_MIN_FAT_FS_SIZE && dataAreaSize <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize())
					{
		...
		case DEVICE_WIPE_MODE_PAGE:
			...
			if (nWipeMode == GST_WIPE_NONE)
				nWipeMode = GST_WIPE_1_RAND;
			...
			else
			{
				...
				// Title bar
				SetWindowText (MainDlg, GST_APP_NAME);
	...
	case GST_APPMSG_PERFORM_POST_SYSENC_WMINIT_TASKS:
	...
	case WM_COMMAND:
		...
		if (lw == IDC_LINK_MORE_INFO_ABOUT_CIPHER && nCurPageNo == CIPHER_PAGE)
		{
			...
			if (strcmp (name, "GOST 28147-89") == 0)
				Extlink("http://www.ietf.org/rfc/rfc5830.txt");
			else if (EAGetCipherCount (nIndex) > 1)
		...
		if (nCurPageNo == SYSENC_RESCUE_DISK_CREATION_PAGE)
		{
			if (lw == IDC_BROWSE)
			{
				char tmpszRescueDiskISO [GST_MAX_PATH+1];
...
BOOL CALLBACK MainDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			strcat (szRescueDiskISO, "\\GostCrypt Rescue Disk.iso");
			...
			PostMessage (hwndDlg, GST_APPMSG_PERFORM_POST_WMINIT_TASKS, 0, 0);
		}
		return 0;
				...
				if (bDeviceWipeInProgress != decoySysPartitionWipeStatus.WipeInProgress)
				{
					...
					if (!bDeviceWipeInProgress)
					{
						...
						if (BootEncStatus.ConfiguredEncryptedAreaEnd == decoySysPartitionWipeStatus.WipedAreaEnd)
						{
							...
							ChangeHiddenOSCreationPhase (GST_HIDDEN_OS_CREATION_PHASE_WIPED);
							...
							TextInfoDialogBox (GST_TBXID_DECOY_OS_INSTRUCTIONS);
	...
	case GST_APPMSG_PERFORM_POST_WMINIT_TASKS:

		AfterWMInitTasks (hwndDlg);
		return 1;

	case GST_APPMSG_FORMAT_FINISHED:
		{
	...
	case GST_APPMSG_NONSYS_INPLACE_ENC_FINISHED:
	...
	case GST_APPMSG_VOL_TRANSFORM_THREAD_ENDED:
	...
	case GST_APPMSG_FORMAT_USER_QUIT:
			...
			&& InstanceHasSysEncMutex ())
		{
				...
				"SYSTEM_DECRYPTION_DEFER_CONFIRM" : "SYSTEM_ENCRYPTION_DEFER_CONFIRM") == IDYES)
			{
				if (nCurPageNo == SYSENC_PRETEST_RESULT_PAGE)
					TextInfoDialogBox (GST_TBXID_SYS_ENC_RESCUE_DISK);
	...
	case WM_COMMAND:
		...
		{
			PostMessage (hwndDlg, GST_APPMSG_FORMAT_USER_QUIT, 0, 0);
			return 1;
		}
		else if (lw == IDC_NEXT)
		{
			...
			else if (nCurPageNo == SYSENC_MULTI_BOOT_MODE_PAGE)
			{
				...
				if (bHiddenOS)
				{
					...
					{
						TextInfoDialogBox (GST_TBXID_EXTRA_BOOT_PARTITION_REMOVAL_INSTRUCTIONS);
						NormalCursor ();
						return 1;
					}
			...
			else if (nCurPageNo == VOLUME_LOCATION_PAGE)
			{
				...
				if (bDevice)
				{
					...
					case -1:
						...
						// non-system devices could not be GST-formatted in such environments). Note that this is
				...
				else
				{
					...
					foreach (const HostDevice &device, GetAvailableHostDevices (true, true))
					{
						if (device.IsPartition || device.DynamicVolume)
						{
							...
							{
								if ((volume.CryptoInfo->HeaderFlags & GST_HEADER_FLAG_NONSYS_INPLACE_ENC) != 0
...
retryCDDriveCheck:
			...
			else if (nCurPageNo == SYSENC_PRETEST_INFO_PAGE)
			{
				...
				if (!bHiddenOS)	// This text is not tailored to hidden OS
					TextInfoDialogBox (GST_TBXID_SYS_ENCRYPTION_PRETEST);
				...
				/* Add the main GostCrypt app to the system startup sequence (the GostCrypt Background Task), which
				we need e.g. for notifications about prevented hibernation, about hidden OS leak protection, about 
				inconsistent hidden OS installs (GostCrypt upgraded in the decoy system but not in the hidden one), etc.
				...
				if (bHiddenOS)
				{
					...
					if (!ChangeSystemEncryptionStatus (SYSENC_STATUS_PRETEST)
						|| !ChangeHiddenOSCreationPhase (GST_HIDDEN_OS_CREATION_PHASE_CLONING))
					{
						ChangeSystemEncryptionStatus (SYSENC_STATUS_NONE);
			...
			else if (nCurPageNo == FORMAT_PAGE)
			{
				...
				if (!bHiddenVol && IsHiddenOSRunning())
				{
					...
					if (fileSystem == FILESYS_NTFS)	
					{
						...
						if (GetVolumeDataAreaSize (FALSE, nVolumeSize) <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize()
				...
				if (bHiddenVolHost)
				{
					...
					if (fileSystem == FILESYS_NTFS)	
					{
						...
						if (fileSystem == FILESYS_NTFS)	// The file system may have been changed in the previous block
						{
							...
							else if (GetVolumeDataAreaSize (FALSE, nVolumeSize) <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize()
...
int DetermineMaxHiddenVolSize (HWND hwndDlg)
{
	...
	if (nbrFreeClusters * realClusterSize < GST_MIN_HIDDEN_VOLUME_SIZE)
	{
	...
	nMaximumHiddenVolSize = nbrFreeClusters * realClusterSize - GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE - nbrReserveBytes;
	nMaximumHiddenVolSize -= nMaximumHiddenVolSize % realClusterSize;		// Must be a multiple of the sector size

	if (nMaximumHiddenVolSize < GST_MIN_HIDDEN_VOLUME_SIZE)
	{
		MessageBoxW (hwndDlg, GetString ("NO_SPACE_FOR_HIDDEN_VOL"), lpszTitle, ICON_HAND);
		UnmountVolume (hwndDlg, hiddenVolHostDriveNo, TRUE);
		AbortProcessSilent ();
...
// the GostCrypt volume format version and the type of volume.
int AnalyzeHiddenVolumeHost (HWND hwndDlg, int *driveNo, __int64 hiddenVolHostSize, int *realClusterSize, __int64 *pnbrFreeClusters)
{
	...
	BYTE readBuffer[GST_MAX_VOLUME_SECTOR_SIZE * 2];
	LARGE_INTEGER offset, offsetNew;
	VOLUME_PROPERTIES_STRUCT volProp;

	memset (&volProp, 0, sizeof(volProp));
	volProp.driveNo = *driveNo;
	if (!DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &volProp, sizeof (volProp), &volProp, sizeof (volProp), &dwResult, NULL) || dwResult == 0)
	{
		handleWin32Error (hwndDlg);
	...
	if (volProp.volFormatVersion < GST_VOLUME_FORMAT_VERSION)
	{
		// We do not support creating hidden volumes within volumes created by GostCrypt 5.1a or earlier.
	...
	if (volProp.volumeHeaderFlags & GST_HEADER_FLAG_NONSYS_INPLACE_ENC
		|| volProp.volumeHeaderFlags & GST_HEADER_FLAG_ENCRYPTED_SYSTEM)
	{
	...
	result = ReadFile (hDevice, &readBuffer, GST_MAX_VOLUME_SECTOR_SIZE, &bytesReturned, NULL);
	...
	else if (!strncmp (szFileSystemNameBuffer, "NTFS", 4))
	{
		...
		if (bHiddenVolDirect && GetVolumeDataAreaSize (FALSE, hiddenVolHostSize) <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize())
		...
		case SYSENC_STATUS_PRETEST:
			{
				...
				bHiddenOS = (hiddenOSCreationPhase != GST_HIDDEN_OS_CREATION_PHASE_NONE);
				...
				if (BootEncStatus.DriveMounted)
				{
					...
					{
					case GST_HIDDEN_OS_CREATION_PHASE_NONE:
					...
					case GST_HIDDEN_OS_CREATION_PHASE_WIPING:
					...
					case GST_HIDDEN_OS_CREATION_PHASE_WIPED:
				...
				else
				{
					...
					if (hiddenOSCreationPhase == GST_HIDDEN_OS_CREATION_PHASE_NONE)
					{
					...
					else
					{
						...
						char *tmpStr[] = {0,
							hiddenOSCreationPhase == GST_HIDDEN_OS_CREATION_PHASE_WIPING ? "OS_WIPING_NOT_FINISHED_ASK" : "HIDDEN_OS_CREATION_NOT_FINISHED_ASK",
...
/* Temp byte to WCHAR function */
/*static void btoc(unsigned char byte, WCHAR *buf)
{
	unsigned char ln, un;
	WCHAR ind[] = {L'0', L'1', L'2', L'3', L'4', L'5', L'6', L'7', L'8', L'9', L'a', L'b', L'c', L'd', L'e', L'f'};
	ln = byte & 0xF;
	un = (byte >> 4) & 0xF;
	buf[0] = ind[un];
	buf[1] = ind[ln];
...
#if GST_MAX_VOLUME_SECTOR_SIZE > 64 * 1024
#error GST_MAX_VOLUME_SECTOR_SIZE > 64 * 1024
#endif
	...
	/* Use TestCRCs and this segment for various debugging tests. This code gets triggered every time the Format application is started */
	/*{
		char crc[STRIBOG_DIGESTSIZE];
		int i;
		TestCRCs(crc);
		WCHAR buf[512];
		for (i = 0; i < 64; i++)
			btoc(crc[i], buf + (i*2));
		buf[i*2] = '\0';
		MessageBoxW (NULL, buf, L"CRC Test", NULL);
	}*/
...
{
	if (!bDevice)
		return GST_SECTOR_SIZE_FILE_HOSTED_VOLUME;
