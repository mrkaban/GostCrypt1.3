...
#define GST_LOCAL_WIN32_WINNT_OVERRIDE	 1
...
#define GST_MAX_NONSYS_INPLACE_ENC_WORK_CHUNK_SIZE	(2048 * BYTES_PER_KB)
#define GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE		(2 * GST_MAX_VOLUME_SECTOR_SIZE)
#define GST_NTFS_CONCEAL_CONSTANT	0xFF
#define GST_NONSYS_INPLACE_ENC_HEADER_UPDATE_INTERVAL	(64 * BYTES_PER_MB)
#define GST_NONSYS_INPLACE_ENC_MIN_VOL_SIZE			(GST_TOTAL_VOLUME_HEADERS_SIZE + GST_MIN_NTFS_FS_SIZE * 2)
...
static __int64 NewFileSysSizeAfterShrink (HANDLE dev, const char *devicePath, int64 *totalClusterCount, DWORD *bytesPerCluster, BOOL silent)
{
	...
	desiredNbrSectors = (fileSysSize - GST_TOTAL_VOLUME_HEADERS_SIZE) / ntfsVolData.BytesPerSector;
...
BOOL CheckRequirementsForNonSysInPlaceEnc (const char *devicePath, BOOL silent)
{
	...
	if (deviceSize < GST_NONSYS_INPLACE_ENC_MIN_VOL_SIZE)
	{
	...
	if (ntfsVolData.FreeClusters.QuadPart * ntfsVolData.BytesPerCluster < GST_TOTAL_VOLUME_HEADERS_SIZE)
	{
		if (!silent)
	...
	if (ntfsVolData.BytesPerSector > GST_MAX_VOLUME_SECTOR_SIZE
...
int EncryptPartitionInPlaceBegin (volatile FORMAT_VOL_PARAMETERS *volParams, volatile HANDLE *outHandle, WipeAlgorithmId wipeAlgorithm)
{
	...
	DWORD dwResult;
	int dwModeOfOperatioID = FIRST_MODE_OF_OPERATION_ID;
	...
	header = (char *) GSTalloc (GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	if (!header)
		return ERR_OUTOFMEMORY;

	VirtualLock (header, GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	...
	while (true)
	{
		...
		if (!DeviceIoControl (dev, FSCTL_SHRINK_VOLUME, &shrinkVolInfo, sizeof (shrinkVolInfo), NULL, 0, &dwResult, NULL))
		{
			...
			if (GetLastError () == ERROR_ACCESS_DENIED)
			{
				...
				{
					if (MoveClustersBeforeThreshold (dev, deviceName, totalClusterCount - (bytesPerCluster > GST_TOTAL_VOLUME_HEADERS_SIZE ? 1 : GST_TOTAL_VOLUME_HEADERS_SIZE / bytesPerCluster)))
					{
	...
	for (int wipePass = 0; wipePass < (wipeAlgorithm == GST_WIPE_NONE ? 1 : PRAND_DISK_WIPE_PASSES); wipePass++)
	{
		/* Switch between ea for choosen the right mode of operation */
		switch (volParams->ea)
		{
		case GOST:
			dwModeOfOperatioID = XTS8;
			break;
		case GRASSHOPPER:
			dwModeOfOperatioID = XTS;
			break;
		default:
			dwModeOfOperatioID = FIRST_MODE_OF_OPERATION_ID;
			break;
		}
		...
		nStatus = CreateVolumeHeaderInMemory (FALSE,
			...
			dataAreaSize,
			0,
			GST_VOLUME_DATA_OFFSET + dataAreaSize,	// Start of the encrypted area = the first byte of the backup heeader (encrypting from the end)
			0,	// No data is encrypted yet
			0,
			volParams->headerFlags | GST_HEADER_FLAG_NONSYS_INPLACE_ENC,
			volParams->sectorSize,
			wipeAlgorithm == GST_WIPE_NONE ? FALSE : (wipePass < PRAND_DISK_WIPE_PASSES - 1));

		if (nStatus != 0)
			goto closing_seq;

		offset.QuadPart = GST_VOLUME_DATA_OFFSET + dataAreaSize;
...
closing_seq:
	...
	burn (header, GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	VirtualUnlock (header, GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	GSTfree (header);
								   ...
								   volatile BOOL *bTryToCorrectReadErrors)
{
	...
	buf = (char *) GSTalloc (GST_MAX_NONSYS_INPLACE_ENC_WORK_CHUNK_SIZE);
	if (!buf)
	{
	...
	if (wipeAlgorithm != GST_WIPE_NONE)
	{
		wipeBuffer = (byte *) GSTalloc (GST_MAX_NONSYS_INPLACE_ENC_WORK_CHUNK_SIZE);
		if (!wipeBuffer)
		{
			nStatus = ERR_OUTOFMEMORY;
			goto closing_seq;
	...
	{
		workChunkSize = (uint32) min (remainingBytes, GST_MAX_NONSYS_INPLACE_ENC_WORK_CHUNK_SIZE);
		...
		unitNo.Value = (remainingBytes - workChunkSize + GST_VOLUME_DATA_OFFSET) / ENCRYPTION_DATA_UNIT_SIZE;
...
inplace_enc_read:
		...
		offset.QuadPart = masterCryptoInfo->EncryptedAreaStart.Value - workChunkSize - GST_VOLUME_DATA_OFFSET;
		...
		if (remainingBytes - workChunkSize < GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE)
		{
			...
			for (i = 0; i < GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE - (remainingBytes - workChunkSize); i++)
				buf[i] ^= GST_NTFS_CONCEAL_CONSTANT;
		}
		...
		if (wipeAlgorithm != GST_WIPE_NONE)
		{
			...
			for (wipePass = 1; wipePass <= GetWipePassCount (wipeAlgorithm); ++wipePass)
			{
					...
					|| WriteFile (dev, wipeBuffer, workChunkSize, &n, NULL) == 0)
				{
					...
					if (workChunkSize > GST_VOLUME_DATA_OFFSET && SetFilePointerEx (dev, offset, NULL, FILE_BEGIN))
					{
						DecryptDataUnits ((byte *) buf, &unitNo, workChunkSize / ENCRYPTION_DATA_UNIT_SIZE, masterCryptoInfo);
						WriteFile (dev, buf + GST_VOLUME_DATA_OFFSET, workChunkSize - GST_VOLUME_DATA_OFFSET, &n, NULL);
					}
		...
		if (lastHeaderUpdateDistance >= GST_NONSYS_INPLACE_ENC_HEADER_UPDATE_INTERVAL)
		{
	...
	if (!bPause)
	{
		...
		for (int wipePass = 0; wipePass < (wipeAlgorithm == GST_WIPE_NONE ? 1 : PRAND_DISK_WIPE_PASSES); wipePass++)
		{
			nStatus = CreateVolumeHeaderInMemory (FALSE,
				...
				masterCryptoInfo->HeaderFlags | GST_HEADER_FLAG_NONSYS_INPLACE_ENC,
			...
			offset.QuadPart = GST_VOLUME_HEADER_OFFSET;
...
int FastVolumeHeaderUpdate (HANDLE dev, CRYPTO_INFO *headerCryptoInfo, CRYPTO_INFO *masterCryptoInfo, __int64 deviceSize)
{
	...
	header = (byte *) GSTalloc (GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	...
	fieldPos = (byte *) header + GST_HEADER_OFFSET_ENCRYPTED_AREA_START;

	offset.QuadPart = deviceSize - GST_VOLUME_HEADER_GROUP_SIZE;

	if (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0
		|| !ReadEffectiveVolumeHeader (TRUE, dev, header, &n) || n < GST_VOLUME_HEADER_EFFECTIVE_SIZE)
	{
		nStatus = ERR_OS_ERROR;
		goto closing_seq;
	}
	...
	fieldPos = (byte *) header + GST_HEADER_OFFSET_HEADER_CRC;
...
{
	char buf [GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE];
	...
	if (ReadFile (dev, buf, GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE, &nbrBytesProcessed, NULL) == 0)
		return ERR_OS_ERROR;

	for (i = 0; i < GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE; i++)
		buf[i] ^= GST_NTFS_CONCEAL_CONSTANT;
	...
	if (WriteFile (dev, buf, GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE, &nbrBytesProcessed, NULL) == 0)
	{
		...
		while (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0
			|| WriteFile (dev, buf, GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE, &nbrBytesProcessed2, NULL) == 0);
...
BOOL SaveNonSysInPlaceEncSettings (int delta, WipeAlgorithmId newWipeAlgorithm)
{
	...
	{
		if (newWipeAlgorithm != GST_WIPE_NONE)
		{
			sprintf (str, "%d", (int) newWipeAlgorithm);

			SaveBufferToFile (str, GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE), strlen(str), FALSE);
		} 
		else if (FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE)))
		{
		...
		return SaveBufferToFile (str, GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC), strlen(str), FALSE);
	}
}
...
int ZeroUnreadableSectors (HANDLE dev, LARGE_INTEGER startOffset, int64 size, int sectorSize, uint64 *zeroedSectorCount)
{
	...
	sectorBuffer = (byte *) GSTalloc (sectorSize);
...
static int OpenBackupHeader (HANDLE dev, const char *devicePath, Password *password, PCRYPTO_INFO *retMasterCryptoInfo, CRYPTO_INFO *headerCryptoInfo, __int64 deviceSize)
{
	...
	if (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0
		|| !ReadEffectiveVolumeHeader (TRUE, dev, (byte *) header, &n) || n < GST_VOLUME_HEADER_EFFECTIVE_SIZE)
	{
		nStatus = ERR_OS_ERROR;
		goto closing_seq;
	}
