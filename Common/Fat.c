...
GetFatParams (fatparams * ft)
{
	...
	if(ft->cluster_size == 0)	// 'Default' cluster size
	{
		...
		if (ft->cluster_size * ft->sector_size > GST_MAX_FAT_CLUSTER_SIZE)
			ft->cluster_size = GST_MAX_FAT_CLUSTER_SIZE / ft->sector_size;

		if (ft->cluster_size > 128)
			ft->cluster_size = 128;
	}

	if (volumeSize <= GST_MAX_FAT_CLUSTER_SIZE * 4)
		ft->cluster_size = 1;
	...
	if(ft->cluster_count >= 65525) // FAT32
	{
		...
		// Align data area on GST_MAX_VOLUME_SECTOR_SIZE

		} while (ft->sector_size == GST_SECTOR_SIZE_LEGACY
				&& (ft->reserved * ft->sector_size + ft->fat_length * ft->fats * ft->sector_size) % GST_MAX_VOLUME_SECTOR_SIZE != 0);
	}
