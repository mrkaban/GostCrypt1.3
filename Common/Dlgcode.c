...
char *LastDialogId;
char szHelpFile[GST_MAX_PATH];
char szHelpFile2[GST_MAX_PATH];
char SecurityTokenLibraryPath[GST_MAX_PATH];
...
/* This mutex is used to prevent users from running the main GostCrypt app or the wizard while an instance
of the GostCrypt installer is running (which is also useful for enforcing restart before the apps can be used). */
...
WipeAlgorithmId nWipeMode = GST_WIPE_NONE;
...
char SysPartitionDevicePath [GST_MAX_PATH];
char SysDriveDevicePath [GST_MAX_PATH];
...
/* Custom class names */
#define GST_DLG_CLASS "CustomDlg"
#define GST_SPLASH_CLASS "SplashDlg"
...
void cleanup ()
{
	...
	if (hDlgClass)
		UnregisterClass (GST_DLG_CLASS, hInst);
	if (hSplashClass)
		UnregisterClass (GST_SPLASH_CLASS, hInst);
	...
	if (hDriver != INVALID_HANDLE_VALUE)
	{
		...
		if (IsNonInstallMode ())
		{
			...
			if (!DeviceIoControl (hDriver, GST_IOCTL_IS_DRIVER_UNLOAD_DISABLED, NULL, 0, &driverUnloadDisabled, sizeof (driverUnloadDisabled), &dwResult, NULL))
...
int FakeDosNameForDevice (const char *lpszDiskFile, char *lpszDosDevice, char *lpszCFDevice, BOOL bNameOnly)
{
	...
	sprintf (lpszDosDevice, "gostcrypt%lu", GetCurrentProcessId ());
	...
	case WM_INITDIALOG:
		{
			...
			// Hyperlink
			SetWindowText (GetDlgItem (hwndDlg, IDC_HOMEPAGE), "www.gostcrypt.org");
			...
			sprintf (szTmp, "GostCrypt %s", VERSION_STRING);
/*
...
*/
			...
			SetDlgItemText (hwndDlg, IDT_ABOUT_RELEASE, GST_STR_RELEASED_BY);
	...
	case WM_COMMAND:
		...
		{
			//Applink ("main", TRUE, "");
			Extlink("http://www.gostcrypt.org/");
			return 1;
		}
...
void PopulateWipeModeCombo (HWND hComboBox, BOOL bNA, BOOL bInPlaceEncryption)
{
	if (bNA)
	{
		AddComboPairW (hComboBox, GetString ("NOT_APPLICABLE_OR_NOT_AVAILABLE"), GST_WIPE_NONE);
	}
	else
	{
		if (bInPlaceEncryption)
			AddComboPairW (hComboBox, GetString ("WIPE_MODE_NONE"), GST_WIPE_NONE);
		else
			AddComboPairW (hComboBox, GetString ("WIPE_MODE_1_RAND"), GST_WIPE_1_RAND);

		AddComboPairW (hComboBox, GetString ("WIPE_MODE_3_DOD_5220"), GST_WIPE_3_DOD_5220);
		AddComboPairW (hComboBox, GetString ("WIPE_MODE_7_DOD_5220"), GST_WIPE_7_DOD_5220);
		AddComboPairW (hComboBox, GetString ("WIPE_MODE_35_GUTMANN"), GST_WIPE_35_GUTMANN);
	}
}
...
void ExceptionHandlerThread (void *threadArg)
{
	...
	case 0xeedfade:
		// Exception not caused by GostCrypt
	...
	// Call stack
	HMODULE dbgDll = LoadLibraryEx ("dbghelp.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
	if (dbgDll)
	{
	...
	sprintf (url, GST_APPLINK_SECURE "&dest=err-report%s&os=%s&osver=%d.%d.%d&arch=%s&cpus=%d&app=%s&cksum=%x&dlg=%s&err=%x&addr=%x"
		, lpack
...
{
	return GSTCreateMutex (&hSysEncMutex, GST_MUTEX_NAME_SYSENC);
}
...
{
	return GSTCreateMutex (&hNonSysInplaceEncMutex, GST_MUTEX_NAME_NONSYS_INPLACE_ENC);
}
...
{
	GSTCloseMutex (&hNonSysInplaceEncMutex);
}
...
BOOL NonSysInplaceEncInProgressElsewhere (void)
{
	return (!InstanceHasNonSysInplaceEncMutex () 
		&& MutexExistsOnSystem (GST_MUTEX_NAME_NONSYS_INPLACE_ENC));
}
...
{
	return GSTCreateMutex (&hDriverSetupMutex, GST_MUTEX_NAME_DRIVER_SETUP);
}


void CloseDriverSetupMutex (void)
{
	GSTCloseMutex (&hDriverSetupMutex);
}


BOOL CreateAppSetupMutex (void)
{
	return GSTCreateMutex (&hAppSetupMutex, GST_MUTEX_NAME_APP_SETUP);
}


void CloseAppSetupMutex (void)
{
	GSTCloseMutex (&hAppSetupMutex);
}


BOOL IsGostCryptInstallerRunning (void)
{
	return (MutexExistsOnSystem (GST_MUTEX_NAME_APP_SETUP));
}


// Returns TRUE if the mutex is (or had been) successfully acquired (otherwise FALSE). 
BOOL GSTCreateMutex (volatile HANDLE *hMutex, char *name)
{
	if (*hMutex != NULL)
...
void GSTCloseMutex (volatile HANDLE *hMutex)
{
	if (*hMutex != NULL)
	{
...
uint32 ReadEncryptionThreadPoolFreeCpuCountLimit ()
{
	...
	if (!ReadLocalMachineRegistryDword ("SYSTEM\\CurrentControlSet\\Services\\gostcrypt", GST_ENCRYPTION_FREE_CPU_COUNT_REG_VALUE_NAME, &count))
		count = 0;
...
BOOL LoadSysEncSettings (HWND hwndDlg)
{
	BOOL status = TRUE;
	DWORD size = 0;
	char *sysEncCfgFileBuf = LoadFile (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION), &size);
	char *xml = sysEncCfgFileBuf;
	char paramName[100], paramVal[MAX_PATH];

	// Defaults
	int newSystemEncryptionStatus = SYSENC_STATUS_NONE;
	WipeAlgorithmId newnWipeMode = GST_WIPE_NONE;

	if (!FileExists (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION)))
	{
...
int LoadNonSysInPlaceEncSettings (WipeAlgorithmId *wipeAlgorithm)
{
	...
	*wipeAlgorithm = GST_WIPE_NONE;

	if (!FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC)))
		return 0;

	if ((fileBuf = LoadFile (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC), &size)) == NULL)
		return 0;

	if (FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE)))
	{
		if ((fileBuf2 = LoadFile (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE), &size2)) != NULL)
...
{
	if (FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC)))
		remove (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC));

	if (FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE)))
		remove (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC_WIPE));
...
void SavePostInstallTasksSettings (int command)
{
	...
	if (IsNonInstallMode() && command != GST_POST_INSTALL_CFG_REMOVE_ALL)
		return;

	switch (command)
	{
	case GST_POST_INSTALL_CFG_REMOVE_ALL:
		remove (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_TUTORIAL));
		remove (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_RELEASE_NOTES));
		break;

	case GST_POST_INSTALL_CFG_TUTORIAL:
		f = fopen (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_TUTORIAL), "w");
		break;

	case GST_POST_INSTALL_CFG_RELEASE_NOTES:
		f = fopen (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_RELEASE_NOTES), "w");
		break;
...
void DoPostInstallTasks (void)
{
	...
	if (FileExists (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_TUTORIAL)))
	{
		if (AskYesNo ("AFTER_INSTALL_TUTORIAL") == IDYES)
			//Applink ("beginnerstutorial", TRUE, "");
			Extlink("http://www.gostcrypt.org/redirect.php?a=2");

		bDone = TRUE;
	}

	if (FileExists (GetConfigPath (GST_APPD_FILENAME_POST_INSTALL_TASK_RELEASE_NOTES)))
	{
	...
	if (bDone)
		SavePostInstallTasksSettings (GST_POST_INSTALL_CFG_REMOVE_ALL);
}
...
#ifndef SETUP
	wc.hIcon = LoadIcon (hInstance, MAKEINTRESOURCE (IDI_GOSTCRYPT_ICON));
#else
#include "../setup/resource.h"
	wc.hIcon = LoadIcon (hInstance, MAKEINTRESOURCE (IDI_SETUP));
#endif
	wc.lpszClassName = GST_DLG_CLASS;
	...
	wc.lpszClassName = GST_SPLASH_CLASS;
	...
	if (LoadLibraryEx("riched20.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32) == NULL)
	{
...
void InitHelpFileName (void)
{
	...
	if (lpszTmp)
	{
		char szTemp[GST_MAX_PATH];

		// Primary file name
		if (strcmp (GetPreferredLangId(), "en") == 0
			|| GetPreferredLangId() == NULL)
		{
			strcpy (++lpszTmp, "GostCrypt User Guide.pdf");
		}
		else
		{
			sprintf (szTemp, "GostCrypt User Guide.%s.pdf", GetPreferredLangId());
...
BOOL OpenDevice (const char *lpszPath, OPEN_TEST_STRUCT *driver, BOOL detectFilesystem)
{
	...
	driver->bDetectGSTBootLoader = FALSE;
	driver->DetectFilesystem = detectFilesystem;

	bResult = DeviceIoControl (hDriver, GST_IOCTL_OPEN_TEST,
	...
	if (bResult == FALSE)
	{
		...
		{
			driver->GSTBootLoaderDetected = FALSE;
...
void NotifyDriverOfPortableMode (void)
{
	if (hDriver != INVALID_HANDLE_VALUE)
	{
		...
		DeviceIoControl (hDriver, GST_IOCTL_SET_PORTABLE_MODE_STATUS, NULL, 0, NULL, 0, &dwResult, NULL);
	}
}
		...
		|| strlen (SysDriveDevicePath) <= 1)
	{
		foreach (const HostDevice &device, GetAvailableHostDevices (false, true))
		{
			if (device.ContainsSystem)
				strcpy_s (device.IsPartition ? SysPartitionDevicePath : SysDriveDevicePath, GST_MAX_PATH, device.Path.c_str()); 
		}
			...
			foreach (const HostDevice &drive, GetAvailableHostDevices (false, false))
			{
				if (drive.ContainsSystem)
				{
					foreach (const HostDevice &sysDrivePartition, drive.Partitions)
					{
						...
						{
							if (sysDrivePartition.Size <= GST_MAX_EXTRA_BOOT_PARTITION_SIZE)
...
BOOL CALLBACK TextInfoDialogBoxDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			switch (nID)
			{
			case GST_TBXID_LEGAL_NOTICES:
				LocalizeDialog (hwndDlg, "LEGAL_NOTICES_DLG_TITLE");
				break;

			case GST_TBXID_SYS_ENCRYPTION_PRETEST:
				LocalizeDialog (hwndDlg, NULL);
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRINT), SW_SHOW);
				break;

			case GST_TBXID_SYS_ENC_RESCUE_DISK:
				LocalizeDialog (hwndDlg, NULL);
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRINT), SW_SHOW);
				break;

			case GST_TBXID_DECOY_OS_INSTRUCTIONS:
				LocalizeDialog (hwndDlg, NULL);
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRINT), SW_SHOW);
				break;

			case GST_TBXID_EXTRA_BOOT_PARTITION_REMOVAL_INSTRUCTIONS:
				LocalizeDialog (hwndDlg, NULL);
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRINT), SW_SHOW);
				break;
			}

			SendMessage (hwndDlg, GST_APPMSG_LOAD_TEXT_BOX_CONTENT, 0, 0);
		}
		return 0;
	...
	case WM_COMMAND:
		...
		if (lw == IDC_PRINT)
		{
			...
			case GST_TBXID_SYS_ENC_RESCUE_DISK:
				PrintHardCopyTextUTF16 ((wchar_t *) GetRescueDiskHelpString ().c_str(), "GostCrypt Rescue Disk Help", GetRescueDiskHelpString ().length () * 2);
				break;
	...
	case GST_APPMSG_LOAD_TEXT_BOX_CONTENT:
		{
			char *r = NULL;
...
BOOL CALLBACK RawDevicesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NOTIFY:
		...
		if (msg == WM_NOTIFY && ((LPNMHDR) lParam)->code == LVN_ITEMCHANGED && (((LPNMLISTVIEW) lParam)->uNewState & LVIS_FOCUSED ))
		{
			...
			LvItem.cchTextMax = GST_MAX_PATH;
		...
		if (msg == WM_COMMAND && lw == IDOK || msg == WM_NOTIFY && ((NMHDR *)lParam)->code == LVN_ITEMACTIVATE)
		{
			...
			strcpy_s (lpszFileName, GST_MAX_PATH, selectedDevice.Path.c_str());
...
BOOL DoDriverInstall (HWND hwndDlg)
{
	...
	hService = OpenService (hManager, "gostcrypt", SERVICE_ALL_ACCESS);
	if (hService == NULL)
		goto error;
...
static int DriverLoad ()
{
	...
	SC_HANDLE hManager, hService = NULL;
	char driverPath[GST_MAX_PATH*2];
	BOOL res;
	char *tmp;
	DWORD startType;

	if (ReadLocalMachineRegistryDword ("SYSTEM\\CurrentControlSet\\Services\\gostcrypt", "Start", &startType) && startType == SERVICE_BOOT_START)
	...
	strcpy (tmp, !Is64BitOs () ? "\\gostcrypt.sys" : "\\gostcrypt-x64.sys");
...
BOOL DriverUnload ()
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_IS_ANY_VOLUME_MOUNTED, NULL, 0, &volumesMounted, sizeof (volumesMounted), &dwResult, NULL);

	if (!bResult)
	{
		bResult = DeviceIoControl (hDriver, GST_IOCTL_LEGACY_GET_MOUNTED_VOLUMES, NULL, 0, &driver, sizeof (driver), &dwResult, NULL);
		if (bResult)
			...
			if (SystemEncryptionStatus != SYSENC_STATUS_NONE)
			{
				...
				if (DoDriverInstall (NULL))
				{
					...
					try
					{
						...
						bootEnc.RegisterBootDriver (bootEnc.GetHiddenOSCreationPhase() != GST_HIDDEN_OS_CREATION_PHASE_NONE ? true : false);
					}
					catch (Exception &e)
					{
						e.Show (NULL);
...
load:
	...
	if (hDriver != INVALID_HANDLE_VALUE)
	{
		...
		BOOL bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_DRIVER_VERSION, NULL, 0, &DriverVersion, sizeof (DriverVersion), &dwResult, NULL);

		if (!bResult)
			bResult = DeviceIoControl (hDriver, GST_IOCTL_LEGACY_GET_DRIVER_VERSION, NULL, 0, &DriverVersion, sizeof (DriverVersion), &dwResult, NULL);
...
{
	OPENFILENAMEW ofn;
	wchar_t file[GST_MAX_PATH] = { 0 };
	wchar_t wInitialDir[GST_MAX_PATH] = { 0 };
	wchar_t filter[1024];
	BOOL status = FALSE;
	...
	*lpszFileName = 0;
	...
	wsprintfW (filter, L"%ls (*.*)%c*.*%c%ls (*.gst)%c*.gst%c%c",
		GetString ("ALL_FILES"), 0, 0, GetString ("GST_VOLUMES"), 0, 0, 0);
...
BOOL SelectMultipleFilesNext (char *lpszFileName)
{
	...
	strncpy (lpszFileName, SelectMultipleFilesPath, GST_MAX_PATH);
	lpszFileName[GST_MAX_PATH - 1] = 0;
...
#ifdef GSTMOUNT
	if (GSTBootLoaderOnInactiveSysEncDrive ())
	{
...
static BOOL PerformBenchmark(HWND hwndDlg)
{
	...
	if (!EAInit (ci->ea, ci->master_keydata, ci->ks))
	{
		...
		if (EAInitMode (ci))
		{
			...
			for (i = 0; i < 10; i++)
			{
				EncryptDataUnits (lpTestBuffer, &startDataUnitNo, (GST_LARGEST_COMPILER_UINT) benchmarkBufferSize / ENCRYPTION_DATA_UNIT_SIZE, ci);
				DecryptDataUnits (lpTestBuffer, &startDataUnitNo, (GST_LARGEST_COMPILER_UINT) benchmarkBufferSize / ENCRYPTION_DATA_UNIT_SIZE, ci);
			}
		}
	}
#endif
...

	{
		...
		for (hid = FIRST_PRF_ID; hid <= LAST_PRF_ID; hid++) 
		{
			...
			case GOSTHASH:
				GOSTHASH_init (&gctx);
				GOSTHASH_add (lpTestBuffer, benchmarkBufferSize, &gctx);
				GOSTHASH_finalize (&gctx, (unsigned char *) digest);
				break;
			case STRIBOG:
				STRIBOG_init (&sctx);
				STRIBOG_add (&sctx, lpTestBuffer, benchmarkBufferSize);
				STRIBOG_finalize (&sctx, (unsigned char *) digest);
				break;
			}
		...
		for (thid = FIRST_PRF_ID; thid <= LAST_PRF_ID; thid++) 
		{
			...
			for (i = 1; i <= 5; i++) 
			{
				...
				case WHIRLPOOL:
					...
					break;
				case GOSTHASH:
					derive_key_gosthash ("passphrase-1234567890", 21, tmp_salt, 64, get_pkcs5_iteration_count(thid, FALSE), dk, MASTER_KEYDATA_SIZE);
					break;
				case STRIBOG:
					derive_key_stribog ("passphrase-1234567890", 21, tmp_salt, 64, get_pkcs5_iteration_count(thid, FALSE), dk, MASTER_KEYDATA_SIZE);
					break;
				}
			}
...
CipherTestDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			SendMessage(GetDlgItem(hwndDlg, IDC_KEY), EM_LIMITTEXT, 256,0);
			...
			SendMessage(GetDlgItem(hwndDlg, IDC_SECONDARY_KEY), EM_LIMITTEXT, 256,0);
	...
	case WM_COMMAND:
		...
		{
			char key[256+1], inputtext[256+1], secondaryKey[256+1], dataUnitNo[16+1], szTmp[256+1];
			...
			if (bXTSTestEnabled)
			{
				...
				if (GetWindowText(GetDlgItem(hwndDlg, IDC_SECONDARY_KEY), szTmp, sizeof(szTmp)) != 64
					&& GetWindowText(GetDlgItem(hwndDlg, IDC_SECONDARY_KEY), szTmp, sizeof(szTmp)) != 256) //Neither 256 or 1024 bit
				{
...
ResetCipherTest(HWND hwndDlg, int idTestCipher)
{
	...
	if (idTestCipher == GOST)
	{
		...
		for (ndx = 0; ndx < BLOCKS_PER_XTS_DATA_UNIT_SMALL; ndx++)
		{
			char tmpStr [16];
	...
	else if (idTestCipher == GRASSHOPPER)
	{
		ndx = SendMessage(GetDlgItem(hwndDlg, IDC_KEY_SIZE), CB_ADDSTRING, 0, (LPARAM) "256");
		SendMessage(GetDlgItem(hwndDlg, IDC_KEY_SIZE), CB_SETITEMDATA, ndx, (LPARAM)32);
		SendMessage(GetDlgItem(hwndDlg, IDC_KEY_SIZE), CB_SETCURSEL, ndx, 0);
		...
		SetWindowText(GetDlgItem(hwndDlg, IDC_KEY), "8899aabbccddeeff0011223344556677fedcba98765432100123456789abcdef");
		SetWindowText(GetDlgItem(hwndDlg, IDC_PLAINTEXT), "1122334455667700ffeeddccbbaa9988");
...
{
	return (WrongPwdRetryCounter > GST_TRY_HEADER_BAK_AFTER_NBR_WRONG_PWD_TRIES);
}
...
int DriverUnmountVolume (HWND hwndDlg, int nDosDriveNo, BOOL forced)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_DISMOUNT_VOLUME, &unmount,
	...
	if (eventId != 0)
	{
		for (i = 0; i < 26; i++)
		{
			if (driveMap & (1 << i))
			{
				...
				{
					char target[32];
					wsprintf (target, "%ls%c", GST_MOUNT_PREFIX, i + 'A');
					root[2] = 0;
...
retry:
	...
	if (path.find ("\\\\?\\") == 0)
	{
		// Remove \\?\ prefix
		path = path.substr (4);
		strcpy_s (volumePath, GST_MAX_PATH, path.c_str());
	}
	...
	if (path.find ("Volume{") == 0 && path.rfind ("}\\") == path.size() - 2)
	{
		...
		if (!resolvedPath.empty())
			strcpy_s (volumePath, GST_MAX_PATH, resolvedPath.c_str());
	}
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_MOUNT_VOLUME, &mount,
			...
			if (bReportWrongPassword)
			{
				...
				if (bDevice && mount.bProtectHiddenVolume)
				{
					...
					if (sscanf (volumePath, "\\Device\\Harddisk%d\\Partition", &driveNo) == 1)
					{
						...
						DWORD dwResult;
						if (DeviceIoControl (hDriver, GST_IOCTL_OPEN_TEST, &openTestStruct, sizeof (OPEN_TEST_STRUCT), &openTestStruct, sizeof (OPEN_TEST_STRUCT), &dwResult, NULL) && openTestStruct.GSTBootLoaderDetected)
...
{
	DWORD dw;
	return !DeviceIoControl (hDriver, GST_IOCTL_GET_PASSWORD_CACHE_STATUS, 0, 0, 0, 0, &dw, 0);
}
...
BOOL IsMountedVolume (const char *volname)
{
	...
	DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, &mlist,
...
BOOL ResolveSymbolicLink (const wchar_t *symLinkName, PWSTR targetName)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_RESOLVED_SYMLINK, &resolve,
...
BOOL GetPartitionInfo (const char *deviceName, PPARTITION_INFORMATION rpartInfo)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_DRIVE_PARTITION_INFO, &dpi,
...
BOOL GetDeviceInfo (const char *deviceName, DISK_PARTITION_INFO_STRUCT *info)
{
	...
	return DeviceIoControl (hDriver, GST_IOCTL_GET_DRIVE_PARTITION_INFO, info, sizeof (*info), info, sizeof (*info), &dwResult, NULL);
}
...
BOOL GetDriveGeometry (const char *deviceName, PDISK_GEOMETRY diskGeometry)
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_DRIVE_GEOMETRY, &dg,
...
BOOL GSTCopyFile (char *sourceFileName, char *destinationFile)
{
	__int8 *buffer;
	HANDLE src, dst;
	FILETIME fileTime;
...
BOOL IsNonInstallMode ()
{
	...
	if (hDriver != INVALID_HANDLE_VALUE)
	{
		...
		if (DeviceIoControl (hDriver, GST_IOCTL_GET_PORTABLE_MODE_STATUS, NULL, 0, NULL, 0, &dw, 0))
		{
			bPortableModeConfirmed = TRUE;
			return TRUE;
		}
	...
	else
	{
		...
		if (hDriverTmp == INVALID_HANDLE_VALUE)
		{
			...
			{
				strcat (path, "\\GostCrypt\\");
				strcat (path, GST_APPD_FILENAME_SYSTEM_ENCRYPTION);
	...
	if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GostCrypt", 0, KEY_READ, &hkey) == ERROR_SUCCESS)
	{
		RegCloseKey (hkey);
		return FALSE;
	}
...
// Adds or removes GostCrypt.exe to/from the system startup sequence (with appropriate command line arguments)
void ManageStartupSeq (void)
{
	if (!IsNonInstallMode ())
	{
		...
		if (bStartOnLogon || bMountDevicesOnLogon || bMountFavoritesOnLogon)
		{
			...
			WriteRegistryString (regk, "GostCrypt", exe);
		}
		else
			DeleteRegistryValue (regk, "GostCrypt");
	}
}


// Adds or removes the GostCrypt Volume Creation Wizard to/from the system startup sequence
	...
	if (!bRemove)
	{
		...
		GetModuleFileName (NULL, exe + 1, sizeof (exe) - 1);
...
#ifndef VOLFORMAT
			{
				...
				if (tmp = strrchr (exe, '\\'))
					strcpy (++tmp, "GostCrypt Format.exe");
			}
#endif
		...
		WriteRegistryString (regk, "GostCrypt Format", exe);
	}
	else
		DeleteRegistryValue (regk, "GostCrypt Format");
}


// Delete the last used Windows file selector path for GostCrypt from the registry
...
BOOL GetMountList (MOUNT_LIST_STRUCT *list)
{
	...
	return DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, list,
...
int GetDriverRefCount ()
{
	...
	bResult = DeviceIoControl (hDriver, GST_IOCTL_GET_DEVICE_REFCOUNT, &refCount, sizeof (refCount), &refCount,
...
void InfoBalloon (char *headingStringId, char *textStringId)
{
	...
	TaskBarIconDisplayBalloonTooltip (MainDlg,
		headingStringId == NULL ? L"GostCrypt" : GetString (headingStringId), 
...
void InfoBalloonDirect (wchar_t *headingString, wchar_t *textString)
{
	...
	TaskBarIconDisplayBalloonTooltip (MainDlg,
		headingString == NULL ? L"GostCrypt" : headingString, 
...
BOOL ConfigWriteBegin ()
{
	...
	if (ConfigBuffer == NULL)
		ConfigBuffer = LoadFile (GetConfigPath (GST_APPD_FILENAME_CONFIGURATION), &size);

	ConfigFileHandle = fopen (GetConfigPath (GST_APPD_FILENAME_CONFIGURATION), "w");
	if (ConfigFileHandle == NULL)
	{
		free (ConfigBuffer);
		ConfigBuffer = NULL;
...
BOOL ConfigWriteEnd ()
{
	...
	GSTFlushFile (ConfigFileHandle);

	CheckFileStreamWriteErrors (ConfigFileHandle, GST_APPD_FILENAME_CONFIGURATION);
...
BOOL LoadDefaultKeyFilesParam (void)
{
	BOOL status = TRUE;
	DWORD size;
	char *defaultKeyfilesFile = LoadFile (GetConfigPath (GST_APPD_FILENAME_DEFAULT_KEYFILES), &size);
...
void DebugMsgBox (char *format, ...)
{
	...
	MessageBox (MainDlg, buf, "GostCrypt debug", 0);
}
...
void Applink (char *dest, BOOL bSendOS, char *extraOutput)
{
	...
	sprintf_s (url, sizeof (url), GST_APPLINK "%s%s&dest=%s", bSendOS ? ("&os=" + GetWindowsEdition()).c_str() : "", extraOutput, dest);
...
void Extlink (char *dest)
{
	char url [MAX_URL_LENGTH];
	
	ArrowWaitCursor();
	
	sprintf_s (url, sizeof(url), dest);
	ShellExecute (NULL, "open", url, NULL, NULL, SW_SHOWNORMAL);
...
BOOL CALLBACK CloseGSTWindowsEnum (HWND hwnd, LPARAM lParam)
{
	if (GetWindowLongPtr (hwnd, GWLP_USERDATA) == (LONG_PTR) 'TRUE')
	{
		char name[1024] = { 0 };
		GetWindowText (hwnd, name, sizeof (name) - 1);
		if (hwnd != MainDlg && strstr (name, "GostCrypt"))
		{
			PostMessage (hwnd, GST_APPMSG_CLOSE_BKG_TASK, 0, 0);
...
BOOL CALLBACK FindGSTWindowEnum (HWND hwnd, LPARAM lParam)
{
	if (*(HWND *)lParam == hwnd)
		return TRUE;
	...
	if (GetWindowLongPtr (hwnd, GWLP_USERDATA) == (LONG_PTR) 'TRUE')
	{
		char name[32] = { 0 };
		GetWindowText (hwnd, name, sizeof (name) - 1);
		if (hwnd != MainDlg && strcmp (name, "GostCrypt") == 0)
		{
			if (lParam != 0)
				*((HWND *)lParam) = hwnd;
		}
...
int OpenVolume (OpenVolumeContext *context, const char *volumePath, Password *password, BOOL write, BOOL preserveTimestamps, BOOL useBackupHeader)
{
	...
	if (context->IsDevice)
	{
		...
		// accidentally quick-formats a dismounted partition-hosted GostCrypt volume as NTFS, etc.)
	...
	for (volumeType = GST_VOLUME_TYPE_NORMAL; volumeType < GST_VOLUME_TYPE_COUNT; volumeType++)
	{
		...
		case GST_VOLUME_TYPE_NORMAL:
			headerOffset.QuadPart = useBackupHeader ? context->HostSize - GST_VOLUME_HEADER_GROUP_SIZE : GST_VOLUME_HEADER_OFFSET;
			break;

		case GST_VOLUME_TYPE_HIDDEN:
			if (GST_HIDDEN_VOLUME_HEADER_OFFSET + GST_VOLUME_HEADER_SIZE > context->HostSize)
				continue;

			headerOffset.QuadPart = useBackupHeader ? context->HostSize - GST_VOLUME_HEADER_SIZE : GST_HIDDEN_VOLUME_HEADER_OFFSET;
			break;
		...
		case GST_VOLUME_TYPE_HIDDEN_LEGACY:
			...
			if (context->IsDevice && deviceGeometry.BytesPerSector != GST_SECTOR_SIZE_LEGACY)
				continue;

			headerOffset.QuadPart = context->HostSize - GST_HIDDEN_VOLUME_HEADER_OFFSET_LEGACY;
			break;
		}
			...
			&& context->IsDevice)
		{
			...
			// partition-hosted GostCrypt volume and then tries to read the embedded backup header.
...
BOOL RemoveDeviceWriteProtection (HWND hwndDlg, char *devicePath)
{
	...
	_snprintf (cmdBatch, sizeof (cmdBatch), "%s\\GostCrypt_Write_Protection_Removal.cmd", temp);
	_snprintf (diskpartScript, sizeof (diskpartScript), "%s\\GostCrypt_Write_Protection_Removal.diskpart", temp);
...
void EnableElevatedCursorChange (HWND parent)
{
	...
	if (UacElevated)
	{
		const char *className = "GostCryptEnableElevatedCursorChange";
		WNDCLASSEX winClass;
		HWND hWnd;
		...
		hWnd = CreateWindowEx (WS_EX_TOOLWINDOW | WS_EX_LAYERED, className, "GostCrypt UAC", 0, 0, 0, GetSystemMetrics (SM_CXSCREEN), GetSystemMetrics (SM_CYSCREEN), parent, NULL, hInst, NULL);
...
{
	OPEN_TEST_STRUCT openTest;
	char upperCasePath[GST_MAX_PATH];
	...
	if (path.find ("\\\\?\\Volume{") == 0 && path.rfind ("}\\") == path.size() - 2)
	{
		char devicePath[GST_MAX_PATH];
		if (QueryDosDevice (path.substr (4, path.size() - 5).c_str(), devicePath, GST_MAX_PATH) != 0)
			return TRUE;
	}
...
std::string VolumeGuidPathToDevicePath (std::string volumeGuidPath)
{
	...
	char volDevPath[GST_MAX_PATH];
	if (QueryDosDevice (volumeGuidPath.substr (0, volumeGuidPath.size() - 1).c_str(), volDevPath, GST_MAX_PATH) == 0)
		return string();
