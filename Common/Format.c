...
#if GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE > 4096
#	error	GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE too large for very small volumes. Revise the code.
#endif

#if GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE_HIGH < GST_MAX_VOLUME_SECTOR_SIZE
#	error	GST_HIDDEN_VOLUME_HOST_FS_RESERVED_END_AREA_SIZE_HIGH too small.
#endif
	...
	{
		reservedSize = GST_TOTAL_VOLUME_HEADERS_SIZE;
	}
...
int GSTFormatVolume (volatile FORMAT_VOL_PARAMETERS *volParams)
{
	int nStatus;
	...
	if (FormatSectorSize < GST_MIN_VOLUME_SECTOR_SIZE
		|| FormatSectorSize > GST_MAX_VOLUME_SECTOR_SIZE
	...
	{
		dataOffset = volParams->hiddenVolHostSize - GST_VOLUME_HEADER_GROUP_SIZE - volParams->size;
	}
	else
	{
		if (volParams->size <= GST_TOTAL_VOLUME_HEADERS_SIZE)
			return ERR_VOL_SIZE_WRONG;

		dataOffset = GST_VOLUME_DATA_OFFSET;
	}
	...
	/* Switch between mode of operations depending on the ea */
	switch (volParams->ea)
	{
	case GOST:
...
begin_format:
	...
	if (volParams->bDevice)
	{
		...
		for (nPass = (bFailedRequiredDASD ? 0 : 1); nPass < 2; nPass++)
		{
			...
			if (nPass == 0)
			{
				char buf [2 * GST_MAX_VOLUME_SECTOR_SIZE];
				DWORD bw;
	...
	else
	{
		...
		if (!volParams->hiddenVol && !bInstantRetryOtherFilesys)
		{
			...
			volumeSize.QuadPart = dataAreaSize + GST_VOLUME_HEADER_GROUP_SIZE;
	...
	if (volParams->hiddenVol)
	{
		...
		if (volParams->hiddenVolHostSize < GST_MIN_HIDDEN_VOLUME_HOST_SIZE || volParams->hiddenVolHostSize > GST_MAX_HIDDEN_VOLUME_HOST_SIZE)
		{		
	...
	else if (bInstantRetryOtherFilesys)
	{
		...
		LARGE_INTEGER offset;
		offset.QuadPart = GST_VOLUME_DATA_OFFSET;
	...
	if (!bInstantRetryOtherFilesys)
	{
		...
		{
			byte buf[GST_VOLUME_HEADER_GROUP_SIZE - GST_VOLUME_HEADER_EFFECTIVE_SIZE];
			DWORD bytesWritten;
...
#if GST_MAX_VOLUME_SECTOR_SIZE > 0xFFFF
#error GST_MAX_VOLUME_SECTOR_SIZE > 0xFFFF
#endif
	...
	// Write header backup
	offset.QuadPart = volParams->hiddenVol ? volParams->hiddenVolHostSize - GST_HIDDEN_VOLUME_HEADER_OFFSET : dataAreaSize + GST_VOLUME_HEADER_GROUP_SIZE;
	...
	/* Switch between mode of operations */
	switch (volParams->ea)
	{
	case GOST:
		dwModeOfOperatioID = XTS8;
...
error:
	...
	if (volParams->fileSystem == FILESYS_NTFS)
	{
		...
		if (retCode != TRUE)
		{
			...
			if (dataAreaSize <= GST_MAX_FAT_SECTOR_COUNT * FormatSectorSize)
			{
				if (AskErrYesNo ("FORMAT_NTFS_FAILED_ASK_FAT") == IDYES)
				{
					...
					volParams->quickFormat = TRUE;		// Volume has already been successfully GST-formatted
...
{
	int write_buf_cnt = 0;
	char sector[GST_MAX_VOLUME_SECTOR_SIZE], *write_buf;
	...
	write_buf = (char *)GSTalloc (FormatWriteBufferSize);
	if (!write_buf)
...
BOOL FormatNtfs (int driveNo, int clusterSize)
{
	...
	PFORMATEX FormatEx;
	HMODULE hModule = LoadLibraryEx("fmifs.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
	int i;
...
static BOOL StartFormatWriteThread ()
{
	...
	WriteThreadBuffer = GSTalloc (FormatWriteBufferSize);
