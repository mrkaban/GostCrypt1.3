...
/* Volume header v5 structure (used since GostCrypt 7.0): */
//
...
/* Deprecated/legacy volume header v4 structure (used by GostCrypt 6.x): */
//
// Offset	Length	Description
// ------------------------------------------
// Unencrypted:
...
/* Deprecated/legacy volume header v3 structure (used by GostCrypt 5.x): */
//
// Offset	Length	Description
// ------------------------------------------
// Unencrypted:
...
/* Deprecated/legacy volume header v2 structure (used before GostCrypt 5.0): */
//
// Offset	Length	Description
// ------------------------------------------
// Unencrypted:
...
{
	char header[GST_VOLUME_HEADER_EFFECTIVE_SIZE];
	KEY_INFO keyInfo;
	...
	GST_EVENT keyDerivationCompletedEvent;
	...
	{
		keyDerivationWorkItems = GSTalloc (sizeof (KeyDerivationWorkItem) * pkcs5PrfCount);
		...
		{
			GSTfree (keyDerivationWorkItems);
			return ERR_OUTOFMEMORY;
		}
		...
		if (encryptionThreadCount > 1)
		{
			...
			while (queuedWorkItems > 0)
			{
				...
				if (queuedWorkItems > 0)
					GST_WAIT_EVENT (keyDerivationCompletedEvent);
			}
			continue;
KeyReady:	;
		}
		else
		{
			...
			case WHIRLPOOL:
				...
				break;
			case STRIBOG:
				derive_key_stribog (keyInfo.userKey, keyInfo.keyLength, keyInfo.salt,
					PKCS5_SALT_SIZE, keyInfo.noIterations, dk, GetMaxPkcs5OutSize());
				break;
			case GOSTHASH:
				derive_key_gosthash (keyInfo.userKey, keyInfo.keyLength, keyInfo.salt,
					PKCS5_SALT_SIZE, keyInfo.noIterations, dk, GetMaxPkcs5OutSize());
				break;
			default:		
				// Unknown/wrong ID
			...
			cryptoInfo->mode++)
		{
				...
				cryptoInfo->ea = EAGetNext (cryptoInfo->ea))
			{
				...
				if (cryptoInfo->mode == XTS || cryptoInfo->mode == XTS8)
				{
				...
				// Header version
				headerVersion = GetHeaderField16 (header, GST_HEADER_OFFSET_VERSION);
				...
				if (!ReadVolumeHeaderRecoveryMode
					&& headerVersion >= 4
					&& GetHeaderField32 (header, GST_HEADER_OFFSET_HEADER_CRC) != GetCrc32 (header + GST_HEADER_OFFSET_MAGIC, GST_HEADER_OFFSET_HEADER_CRC - GST_HEADER_OFFSET_MAGIC))
					continue;

				// Required program version
				cryptoInfo->RequiredProgramVersion = GetHeaderField16 (header, GST_HEADER_OFFSET_REQUIRED_VERSION);
				cryptoInfo->LegacyVolume = cryptoInfo->RequiredProgramVersion < 0x600;

				// Check CRC of the key set
				if (!ReadVolumeHeaderRecoveryMode
					&& GetHeaderField32 (header, GST_HEADER_OFFSET_KEY_AREA_CRC) != GetCrc32 (header + HEADER_MASTER_KEYDATA_OFFSET, MASTER_KEYDATA_SIZE))
					continue;
				...
				cryptoInfo->volume_creation_time = GetHeaderField64 (header, GST_HEADER_OFFSET_VOLUME_CREATION_TIME).Value;

				// Header creation time (legacy)
				cryptoInfo->header_creation_time = GetHeaderField64 (header, GST_HEADER_OFFSET_MODIFICATION_TIME).Value;

				// Hidden volume size (if any)
				cryptoInfo->hiddenVolumeSize = GetHeaderField64 (header, GST_HEADER_OFFSET_HIDDEN_VOLUME_SIZE).Value;

				// Hidden volume status
				cryptoInfo->hiddenVolume = (cryptoInfo->hiddenVolumeSize != 0);

				// Volume size
				cryptoInfo->VolumeSize = GetHeaderField64 (header, GST_HEADER_OFFSET_VOLUME_SIZE);
				
				// Encrypted area size and length
				cryptoInfo->EncryptedAreaStart = GetHeaderField64 (header, GST_HEADER_OFFSET_ENCRYPTED_AREA_START);
				cryptoInfo->EncryptedAreaLength = GetHeaderField64 (header, GST_HEADER_OFFSET_ENCRYPTED_AREA_LENGTH);

				// Flags
				cryptoInfo->HeaderFlags = GetHeaderField32 (header, GST_HEADER_OFFSET_FLAGS);

				// Sector size
				if (headerVersion >= 5)
					cryptoInfo->SectorSize = GetHeaderField32 (header, GST_HEADER_OFFSET_SECTOR_SIZE);
				else
					cryptoInfo->SectorSize = GST_SECTOR_SIZE_LEGACY;

				if (cryptoInfo->SectorSize < GST_MIN_VOLUME_SECTOR_SIZE
					|| cryptoInfo->SectorSize > GST_MAX_VOLUME_SECTOR_SIZE
...
ret:
	...
	if (encryptionThreadCount > 1)
	{
		GST_WAIT_EVENT (noOutstandingWorkItemEvent);
...
static char itoc_(unsigned char input)
{
	if (input >= 0x0 && input <= 0x9)
		return (char)input + 0x30;
	else if (input >= 0xA && input <= 0xF)
		return (char)input + 0x61 - 0xA;
	else
		return '?';
}
...
{
//dk and masterKey need more room if cascading ciphers are allowed
	char dk[32 * 2];		// 2 * 256-bit key
	char masterKey[32 * 2]; // 2 * 256-bit key
	PCRYPTO_INFO cryptoInfo;
	int status;
	...
	// PKCS5 PRF used when booting
	derive_key_stribog (password->Text, (int) password->Length, header + HEADER_SALT_OFFSET,
		PKCS5_SALT_SIZE, get_pkcs5_iteration_count(STRIBOG, bBoot), dk, sizeof (dk));
	...
	for (cryptoInfo->ea = EAGetFirst (); cryptoInfo->ea != 0; cryptoInfo->ea = EAGetNext (cryptoInfo->ea))
	{
		...
		if (GetHeaderField32 (header, GST_HEADER_OFFSET_MAGIC) != 0x54525545
			|| (GetHeaderField16 (header, GST_HEADER_OFFSET_VERSION) >= 4 && GetHeaderField32 (header, GST_HEADER_OFFSET_HEADER_CRC) != GetCrc32 (header + GST_HEADER_OFFSET_MAGIC, GST_HEADER_OFFSET_HEADER_CRC - GST_HEADER_OFFSET_MAGIC))
			|| GetHeaderField32 (header, GST_HEADER_OFFSET_KEY_AREA_CRC) != GetCrc32 (header + HEADER_MASTER_KEYDATA_OFFSET, MASTER_KEYDATA_SIZE))
		{
		...
		// Hidden volume status
		cryptoInfo->VolumeSize = GetHeaderField64 (header, GST_HEADER_OFFSET_HIDDEN_VOLUME_SIZE);
		   ...
		   unsigned __int64 encryptedAreaStart, unsigned __int64 encryptedAreaLength, uint16 requiredProgramVersion, uint32 headerFlags, uint32 sectorSize, BOOL bWipeMode)
{
	...
	memset (header, 0, GST_VOLUME_HEADER_EFFECTIVE_SIZE);
	...
	{
		// We have no master key data (creating a new volume) so we'll use the GostCrypt RNG to generate them
	...
	{
	default:
		mputWord (p, requiredProgramVersion != 0 ? requiredProgramVersion : GST_VOLUME_MIN_REQUIRED_PROGRAM_VERSION);
	}
	...
	x = GetCrc32 (header + GST_HEADER_OFFSET_MAGIC, GST_HEADER_OFFSET_HEADER_CRC - GST_HEADER_OFFSET_MAGIC);
	p = header + GST_HEADER_OFFSET_HEADER_CRC;
	mputLong (p, x);
...
{
#if GST_VOLUME_HEADER_EFFECTIVE_SIZE > GST_MAX_VOLUME_SECTOR_SIZE
#error GST_VOLUME_HEADER_EFFECTIVE_SIZE > GST_MAX_VOLUME_SECTOR_SIZE
#endif

	byte sectorBuffer[GST_MAX_VOLUME_SECTOR_SIZE];
	DISK_GEOMETRY geometry;

	if (!device)
		return ReadFile (fileHandle, header, GST_VOLUME_HEADER_EFFECTIVE_SIZE, bytesRead, NULL);

	if (!DeviceIoControl (fileHandle, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &geometry, sizeof (geometry), bytesRead, NULL))
		return FALSE;

	if (geometry.BytesPerSector > sizeof (sectorBuffer) || geometry.BytesPerSector < GST_MIN_VOLUME_SECTOR_SIZE)
	{
		SetLastError (ERROR_INVALID_PARAMETER);
		return FALSE;
	}

	if (!ReadFile (fileHandle, sectorBuffer, max (GST_VOLUME_HEADER_EFFECTIVE_SIZE, geometry.BytesPerSector), bytesRead, NULL))
		return FALSE;

	memcpy (header, sectorBuffer, min (*bytesRead, GST_VOLUME_HEADER_EFFECTIVE_SIZE));
	
	if (*bytesRead > GST_VOLUME_HEADER_EFFECTIVE_SIZE)
		*bytesRead = GST_VOLUME_HEADER_EFFECTIVE_SIZE;
...
#error GST_VOLUME_HEADER_EFFECTIVE_SIZE > GST_MAX_VOLUME_SECTOR_SIZE
	...
	if (!device)
	{
		if (!WriteFile (fileHandle, header, GST_VOLUME_HEADER_EFFECTIVE_SIZE, &bytesDone, NULL))
			return FALSE;

		if (bytesDone != GST_VOLUME_HEADER_EFFECTIVE_SIZE)
		{
			SetLastError (ERROR_INVALID_PARAMETER);
			return FALSE;
		}
	...
	if (geometry.BytesPerSector != GST_VOLUME_HEADER_EFFECTIVE_SIZE)
	{
	...
	memcpy (sectorBuffer, header, GST_VOLUME_HEADER_EFFECTIVE_SIZE);
...
int WriteRandomDataToReservedHeaderAreas (HANDLE dev, CRYPTO_INFO *cryptoInfo, uint64 dataAreaSize, BOOL bPrimaryOnly, BOOL bBackupOnly)
{
	...
	byte buf[GST_VOLUME_HEADER_GROUP_SIZE];
	...
	while (TRUE)
	{
		...
		offset.QuadPart = backupHeaders ? dataAreaSize + GST_VOLUME_HEADER_GROUP_SIZE : GST_VOLUME_HEADER_OFFSET;
		...
		EncryptBuffer (buf + GST_VOLUME_HEADER_EFFECTIVE_SIZE, sizeof (buf) - GST_VOLUME_HEADER_EFFECTIVE_SIZE, cryptoInfo);
...
#endif // !defined (DEVICE_DRIVER) && !defined (GST_WINDOWS_BOOT)
