		...
		case IOCTL_DISK_IS_WRITABLE:
			{
				...
				return GSTCompleteDiskIrp (Irp, STATUS_MEDIA_WRITE_PROTECTED, 0);
			}

		case GST_IOCTL_DISK_IS_WRITABLE:
			Dump ("GST_IOCTL_DISK_IS_WRITABLE pdo=%p\n", Extension->Pdo);

			if (!ProbingHostDeviceForWrite)
				break;

			// Probe the real state of the device as the user is mounting a GostCrypt volume.

			// Volume filter may be attached to a merged drive+volume PDO. First test if GST_IOCTL_DISK_IS_WRITABLE works for the underlying device.
			status = SendDeviceIoControlRequest (Extension->LowerDeviceObject, GST_IOCTL_DISK_IS_WRITABLE, NULL, 0, NULL, 0);

			if (NT_SUCCESS (status) || status == STATUS_MEDIA_WRITE_PROTECTED)
			{
				IoReleaseRemoveLock (&Extension->Queue.RemoveLock, Irp);
				return GSTCompleteDiskIrp (Irp, status, 0);
			}
