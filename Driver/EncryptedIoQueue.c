	...
	while (TRUE)
	{
		...
		for (buffer = queue->FirstPoolBuffer; ; buffer = buffer->NextBuffer)
		{
			...
			if (!buffer || !buffer->NextBuffer)
			{
				...
				newBuffer = GSTalloc (sizeof (EncryptedIoQueueBuffer));
				if (!newBuffer)
				{
					bufferAddress = NULL;
					break;
				...
				bufferAddress = GSTalloc (requestedSize);
				if (bufferAddress)
				{
...
static NTSTATUS CompleteOriginalIrp (EncryptedIoQueueItem *item, NTSTATUS status, ULONG_PTR information)
{
	...
	GSTCompleteDiskIrp (item->OriginalIrp, status, information);
...
static NTSTATUS GSTCachedRead (EncryptedIoQueue *queue, IO_STATUS_BLOCK *ioStatus, PVOID buffer, LARGE_INTEGER offset, ULONG length)
{
	...
	if (queue->IsFilterDevice)
		return GSTReadDevice (queue->LowerDeviceObject, buffer, offset, length);
				...
				if (queue->IsFilterDevice)
				{
					if (queue->RemapEncryptedArea && request->EncryptedLength > 0)
					{
							...
							for (subFragment = 0 ; subFragment < 3; ++subFragment)
							{
								...
								if (subFragmentLength > 0)
								{
									if (request->Item->Write)
										request->Item->Status = GSTWriteDevice (queue->LowerDeviceObject, subFragmentData, subFragmentOffset, subFragmentLength);
									else
										request->Item->Status = GSTCachedRead (queue, NULL, subFragmentData, subFragmentOffset, subFragmentLength);
						...
						else
						{
							...
							if (request->Item->Write)
								request->Item->Status = GSTWriteDevice (queue->LowerDeviceObject, request->Data, remappedOffset, request->Length);
							else
								request->Item->Status = GSTCachedRead (queue, NULL, request->Data, remappedOffset, request->Length);
						}
					}
					else
					{
						if (request->Item->Write)
							request->Item->Status = GSTWriteDevice (queue->LowerDeviceObject, request->Data, request->Offset, request->Length);
						else
							request->Item->Status = GSTCachedRead (queue, NULL, request->Data, request->Offset, request->Length);
					}
				}
				else
				{
					...
					if (request->Item->Write)
						...
						request->Item->Status = GSTCachedRead (queue, &ioStatus, request->Data, request->Offset, request->Length);
			...
			else
			{
				...
				if (readAhead)
				{
					...
					if (queue->ReadAheadOffset.QuadPart + queue->ReadAheadLength <= queue->MaxReadAheadOffset.QuadPart)
					{
						...
						if (queue->IsFilterDevice)
						{
							queue->ReadAheadBufferValid = NT_SUCCESS (GSTReadDevice (queue->LowerDeviceObject, queue->ReadAheadBuffer, queue->ReadAheadOffset, queue->ReadAheadLength));
						}
						else
						{
							IO_STATUS_BLOCK ioStatus;
	...
	while (!queue->ThreadExitRequested)
	{
		...
		while ((listEntry = ExInterlockedRemoveHeadList (&queue->MainThreadQueue, &queue->MainThreadQueueLock)))
		{
				...
				&& (item->OriginalOffset.QuadPart & (ENCRYPTION_DATA_UNIT_SIZE - 1)) != 0)
			{
				...
				item->Status = GSTReadDevice (queue->LowerDeviceObject, buffer, alignedOffset, alignedLength);
			...
			if (!queue->IsFilterDevice)
			{
				...
				&& RegionsOverlap (item->OriginalOffset.QuadPart, item->OriginalOffset.QuadPart + item->OriginalLength - 1, GST_BOOT_VOLUME_HEADER_SECTOR_OFFSET, GST_BOOT_VOLUME_HEADER_SECTOR_OFFSET + GST_BOOT_ENCRYPTION_VOLUME_HEADER_SIZE - 1))
			{
				...
				&& (RegionsOverlap (item->OriginalOffset.QuadPart, item->OriginalOffset.QuadPart + item->OriginalLength - 1, GST_SECTOR_SIZE_BIOS, GST_BOOT_LOADER_AREA_SECTOR_COUNT * GST_SECTOR_SIZE_BIOS - 1)
			...
			{
				BOOL isLastFragment = dataRemaining <= GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE;
				
				ULONG dataFragmentLength = isLastFragment ? dataRemaining : GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE;
				...
				dataRemaining -= GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE;
				dataBuffer += GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE;
				fragmentOffset.QuadPart += GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE;
			}
		}
	}
...
NTSTATUS EncryptedIoQueueStart (EncryptedIoQueue *queue)
{
	...
	queue->FragmentBufferA = GSTalloc (GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE);
	if (!queue->FragmentBufferA)
		goto noMemory;

	queue->FragmentBufferB = GSTalloc (GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE);
	...
	queue->ReadAheadBufferValid = FALSE;
	queue->ReadAheadBuffer = GSTalloc (GST_ENC_IO_QUEUE_MAX_FRAGMENT_SIZE);
	if (!queue->ReadAheadBuffer)
		goto noMemory;

	// Preallocate buffers
	for (i = 0; i < GST_ENC_IO_QUEUE_PREALLOCATED_IO_REQUEST_COUNT; ++i)
	{
		if (i < GST_ENC_IO_QUEUE_PREALLOCATED_ITEM_COUNT && !GetPoolBuffer (queue, sizeof (EncryptedIoQueueItem)))
			goto noMemory;
	...
	status = GSTStartThread (MainThreadProc, queue, &queue->MainThread);
	if (!NT_SUCCESS (status))
		goto err;
	...
	status = GSTStartThread (IoThreadProc, queue, &queue->IoThread);
	if (!NT_SUCCESS (status))
	{
		queue->ThreadExitRequested = TRUE;
		GSTStopThread (queue->MainThread, &queue->MainThreadQueueNotEmptyEvent);
		goto err;
	}
	...
	status = GSTStartThread (CompletionThreadProc, queue, &queue->CompletionThread);
	if (!NT_SUCCESS (status))
	{
		queue->ThreadExitRequested = TRUE;
		GSTStopThread (queue->MainThread, &queue->MainThreadQueueNotEmptyEvent);
		GSTStopThread (queue->IoThread, &queue->IoThreadQueueNotEmptyEvent);
		goto err;
	}
...
err:
	if (queue->FragmentBufferA)
		GSTfree (queue->FragmentBufferA);
	if (queue->FragmentBufferB)
		GSTfree (queue->FragmentBufferB);
	if (queue->ReadAheadBuffer)
		GSTfree (queue->ReadAheadBuffer);
...
NTSTATUS EncryptedIoQueueStop (EncryptedIoQueue *queue)
{
	...
	GSTStopThread (queue->MainThread, &queue->MainThreadQueueNotEmptyEvent);
	GSTStopThread (queue->IoThread, &queue->IoThreadQueueNotEmptyEvent);
	GSTStopThread (queue->CompletionThread, &queue->CompletionThreadQueueNotEmptyEvent);
